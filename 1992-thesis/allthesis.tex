...
\vskip.15pt
\begin{enumerate}
\item{i){\phantom{i}}}\   $z(W,{\alpha})+z(W',{\alpha})=0$ where $W$ and $W'$ are adjacent walls 
separated by a wall of type ${\alpha}$, and
\item{ii)}\   $\exp(z_{p}X_{-\alpha_p})\ldots
...
\begin{enumerate}
\item{i'){\phantom{i}}}\   ${\omega}_{{\alpha}}^{2}=1$
\item{ii')}\   $(\omega_{\alpha}\omega_{\beta})^{m_{\alpha\beta}} = 1$
...
\begin{enumerate}
\item{1)}\   $\exp(z_{q}X_{-q})\ldots \exp(z_{1}X_{-1})\quad
(X_{-i}\ {\mathrel{\mathop=^{def}}}\ X_{-\alpha_i})$\ and
\item{2)}\ ${\omega}_{q}\ldots {\omega}_{1}$.
\end{enumerate}
...
\section{{4.\  The\ Morphism\ $S_1 \to S$}}  
...
\begin{proposition}{Proposition\ 5.2}   Let $R$ be the field of rational functions
...
\chapter{II. Coordinates and Coordinate Relations} 
...
\section{ 1.\  The\ Coordinates\ $x(W,\beta)$ }
...
\begin{remark*}{Remark}    This lemma will complete the proof of lemma 3.2, for 
$$
...
\cite{\LL,3.1} gives an algorithm to solve for $n_{w}$ and hence for 
...
\section{5.\   A\ Computation\ of\  $t^{-1}n^{-1}tn$}
...
in terms of the variables $\{z_1(W,\alpha)\}$\  and  $t$.
   ...
   We need a more precise formulation of \cite{\LL,5.3}. 
...
${\gamma}$. 
 Implicit in \cite{\LL,5.3} is a formula for $t$. 
...
\begin{remark*}{Remark}   The result is highly dependent on the order of the products, on
the choices of root vectors, and the choices of Weyl group representatives.
\end{remark*}
...
\section{8.\ The\ Functions\ $n_\gamma$}
...
\section{9.\ The\  Fundamental\  Divisors\  on\ $Y_\Gamma$}
...
(= z_{1}(W,{\alpha})/z_{1}(W',{\alpha})) = 0$ on $E$, then $E$ is a spurious divisor. 
 By \cite{\LL,3.8} the condition that $z(W,{\alpha})/z(W',{\alpha}) = 0$ is 
...
be the multiplicity with which ${\lambda}$ vanishes along $E$, equals one.
   ...
   Spaltenstein \cite{\Spal,3.2} tells us that $\dim C_{G}(v) = \dim(M_{{\Sigma}})$ 
...
obtain the unipotent radical of a Borel subgroup of $A_{2}$. 
 These facts follow by an examination of the structure constants \cite{\Hu,33.5}. 
...
\section{3.\  Exclusion\ of\ Spurious\ Divisors}
...
\definition{Assumption\ 3.1} $\vert S\vert \ge 2$ and if ${\alpha},
...
\definition{Fact\  1} If $\bWW$ is an ${\alpha}$-chamber in
$L^-$, then $\bWW$\ $ \cap  L^{+}$ contains a big wall.
\enddefinition


\definition{Fact\ 2}   (Assume ${\alpha}\ne {\alpha}_{+},{\alpha}_{-}$ for
...
\definition{Fact\ 3}  (Assume $\alpha\ne\alpha_n$ for $B_{n},C_{n}; {\alpha} \ne
...
\definition{Fact\ 4} (Assume $\alpha\ne\alpha_n$ for $B_n$,$C_n$;
${\alpha}\ne {\alpha}_{+},{\alpha}_{-}$ for
...
\section{2.\  Igusa\ theory\ and\ measures}
...
\section{3.\ Principal\ value\ integrals\ at\ points\ of\
...
\medskip
\noindent
A.i)\ \ \ $y_{1} = y_{2}$ on $U_{1} \cap U_{2}$
...
\smallskip
\noindent
A.iii)\  If $E_{0}$ intersects $U_{1}$ or $U_{2}$ then $x_{1} = 0$ defines
...
\begin{enumerate}
\item{i)} $U$ does not intersect any divisors other than $E_{1}$ and $E_{2}$.
\item{ii)} ${\mu}_{1} = 0$ defines $E_{1}$ and ${\mu}_{2} = 0$ defines $E_{2}$.
\item{iii)} $f$ is locally constant on $U(F)$.
\item{iv)} ${\omega}_{Y} =
{\gamma}{\mu}_{1}^{b}{\mu}_{2}^{b}d{\mu}_{1}d{\mu}_{2}\ldots .d{\mu}_{n}$
with $\vert {\gamma}\vert$ constant on $U$.
\item{v)} $U(K) = \{({\mu}_{1},\ldots ,{\mu}_{n}): \vert {\mu}_{i}\vert \le
q^{-m_{i}} \}$ and $m_{1} = m_{2}$.
\item{vi)} $U(F) = \{({\mu}_{1},\ldots ,{\mu}_{n}) \in U(K) :
{\sigma}({\mu}_{1}) = {\mu}_{2}, {\sigma}({\mu}_{i}) = {\mu}_{i} \ i \ge 3\} $
$$({\sigma} \in \GalK, {\sigma}\ne 1).$$
\item{vii)} ${\lambda} = {\alpha}{\mu}_{1}{\mu}_{2}$ with $\vert {\alpha}\vert$
constant on $U(K)$.
\end{enumerate}
...
\definition{Case\ 1} ${\alpha}' = {\alpha''}$.  We appeal to the $2\times 2$ matrix
calculation
$$
...
\definition{Case\ 2}\  $({\alpha}',{\alpha}'') = 0$.  $x({\alpha}')  = 
...
\operatorname{Gal}(F_{{\alpha}}/F)$.  Then by \cite{\LL,5.4} and its generalization in
...
\chapter{VII. Applications to Endoscopic Groups}
...
\begin{enumerate}
\item{1)}\ The identity component of the dual $^{L}H^{0}$ of $H$ is the connected
centralizer of a semisimple element $s  \in  {}^{L}G^{0}$.
\item{2)}\  There is a homomorphism ${\rho }$ from $\Gal$ to the group
...
\subsection{ 1.1.\ The \ groups\  ${}^{1}A_{n}$\ and\ ${}^{2}A_n$}
...
\subsection{{1.6.\ Other\  exceptional\  groups}}

   We will not compute these.  It should be pointed out however that most
of the cuspidal endoscopic groups can be deduced directly from \cite{\GR} where
...
\begin{proof} \ This is proved in \cite{\GR}.  The algebra $A_{2}\oplus D_{5}$
...
\section{2.\ Characters,\ Centers,\ and\ Endoscopic\ Groups}
...
\cite{\Ba}.  Let $\tilde x  =  {}^{L}{\varphi}(x) {\ \forall\ } x  \in 
...
\definition{${}^{2}A_{n}$}  Assuming that $X(H)  \ne {\phi}$, we may take $G$ to
...
\definition{$C_{n}$} $X(H)  =  Y(T,{\kappa})  =  \{1\}$.\enddefinition

\definition{$D_{n},{}^{2}D_{n}$}  $n$ even.  $Y(T,{\kappa})  =  \{1\}$ and we never
obtain an even rank unitary group.  
\enddefinition

\definition{$D_{n},{}^{2}D_{n}$} $n$ odd.  $Y(T,{\kappa})  =  \{{\theta}^{2}\}$.
...
\definition{$E_{7}$} We take the dual of $E_{7}$ to be the adjoint group of type
...
\definition{${}^{2}E_{6}$}  We turn our attention to the group ${}^{2}E_{6}$.  I am
...
\subsection{5.1.\ Vanishing\ of\ Germs}
...
radical \cite{\Sl,7.5}.  Write $g = g_{0}n$ according to this semi-direct
product.
$$
...
\begin{remark*}{Remark} This centralizer argument can be applied quite
...
following data:
\begin{enumerate}
\item{1.}\   A surface $S$ (together with a description of its components and
rationality structure)
\item{2.}\  a $2$-form on $S$ defined over $F$.
\item{3.}\  a cocycle $b_{{\sigma}}$ in $T$ depending on $p  \in  S(F)$
\item{4.}\  a character ${\kappa}$ on $T$
\item{5.}\  canonical coordinates $(w,{\xi})$.
\end{enumerate}
...
\begin{remark*}{Remark} We make a few observations about the transfer.
...
\ref\no 1
\by J. Arthur 
\paper On some problems suggested by the Trace Formula. 
\book Lie Group Representations II
\inbook Lecture Notes in Math., vol. 1041
...
\bookinfo Proceedings of Symposia in Pure Mathematics, Volume XXXIII, Part 2 
...
\publ Wiley-Interscience, John Wiley and Sons
\publaddr  Chichester 
\yr 1985 
\sendref
 
\ref\no 4
\by M. Golubitsky and B. Rothschild
\paper Primitive Subalgebras of Exceptional Lie Algebras 
\jour Pacific Journal of Mathematics
\vol 39 No. 2
\yr 1971
\sendref
...
\ref\no 5
\by Harish-Chandra
\paper Admissible invariant distributions on reductive $p$-adic groups 
\jour Queen's Papers in Pure and Applied Math.
\vol   48
\pages 281--347 
\yr 1978
\sendref
...
\by H. Hironaka
\book Algebraic Geometry
\bookinfo J.J.  Sylvester Sympos. Johns Hopkins Univ. 1976
\publaddr Baltimore, Md. 
\yr 1977
\pages 52--125.  
\publ Johns Hopkins Univ. Press
\sendref 

\ref\no 9
\by H. Hironaka
\book Memorias de Matematica del Instituto ``Jorge Juan''
\vol 29  
\publ Consejo Superior de Investigaciones Cientificas
\publaddr  Madrid 
\yr 1975 
\sendref
...
\ref\no 11
\by J.-I. Igusa 
\book Lectures on forms of higher degree 
...
\ref\no 13
\by M. Kneser 
\paper Galois-Kohomologie halbeinfacher algebraischer Gruppen 
\"uber \hfil\break $p$-adischen K\"orpern  
\jour Math. Z. 
\vol 88
\yr 1965 
\pages 40--47 
...
\ref\no 14
\by R. Langlands
\book Les debuts d'une formule des traces stable
\publ  Publications math. de l'Univ. de Paris VII
\vol 13
\yr 1983
\sendref 
...
\ref\no 15
\by R. Langlands
\paper Orbital Integrals on Forms of $SL(3)$, I
...
\paper On Principal Values on $p$-adic Manifolds
...
\ref\no 17
\by R. Langlands and D. Shelstad
\paper On the Definition of the Transfer Factors
\jour Math. Ann. 
\vol 278
\pages 219--271
\yr 1987
...
\by J.-P. Serre 
\book Corps Locaux
\bookinfo  Actualit\'es scientifiques et industrielles 
\vol 1296
\publ Hermann 
\publaddr Paris 
\yr 1962 
...
\ref\no 23
\by R. Steinberg
\book Conjugacy Classes in Algebraic Groups
\bookinfo Lecture Notes in Math.
\vol 366.  
\publ Springer-Verlag
\publaddr Berlin
...
\blank
\lish{$\alpha$            }{  I.2      }{  simple root}
\lish{$\alpha$            }{  IV.7      }{  root next in size to $\beta$ in $S_-$}
\lish{$\alpha$-cell       }{  IV.5}
\lish{$\alpha$-wall       }{  IV.4}
\lish{$\alpha$-chamber    }{  IV.4}
\lish{$\alpha$            }{  II.7      }{  short simple root of $G_2$}
\lish{$\alpha$            }{  III.1     }{  positive simple root in a rank two group}
\lish{$\alpha^v$          }{  I.5       }{  coroot associated to $\alpha$}
\blank
\lish{$\beta$             }{            }{  positive simple root}
\lish{$\beta$             }{  II.7      }{  long simple root of $G_2$}
\lish{$\beta$             }{  III.1     }{  positive simple root in a rank two group}
\lish{$\beta$             }{  IV.7      }{  largest simple root of $S_-$}
\lish{$\beta(E_\Sigma)$   }{  II.9      }{  Igusa constants}
\lish{$\beta$             }{  V.3       }{  Igusa constant, see Langlands [\LL]}
\blank
\lish{$\gamma$            }{  III.1     }{  positive root in a rank two group}
\lish{$\gamma$            }{  II.7      }{  root $\alpha+\beta$ of $G_2$}
\lish{$\gamma_j$          }{  VII.5.13  }{  root $T_j-T_{-j}$}
\lish{$\Gamma$            }{  I.1,I.6       }{  regular curve in $T$}
\lish{$\Gamma$            }{  I.6       }{  $\Gamma\setminus \{0\}$}
\lish{$\Gamma_O,\Gamma_{O'}$    }{      VII.4 }{         subregular germs}
\lish{$\Gamma_0$          }{  IV.3.2    }{  a graph}
\lish{$\Gamma_1$          }{  IV.4      }{  minimal tree in $\Gamma_0$ containing extremal }
\lish{}{}{\quad $\alpha,\beta\in S_-$}
\lish{$\Gamma'$           }{  IV.4      }{  pruned $\Gamma_1$, as a graph it equals $S_-$}
\blank
\lish{$\delta$            }{  III.1     }{  positive root in a rank two group}
\lish{$\delta$            }{  II.7      }{  root $2\alpha+\beta$ of $G_2$}
\lish{$\delta$            }{  V.2.3     }{  $\delta(\xi) = x(\beta)/\xi x(\gamma)$}
\lish{$\Delta_\Gamma$     }{  VI.2      }{  $=m_\kappa(e')$, $e' = e(T(W,\alpha)$ : $\alpha$ simple}
\lish{$\Delta'$           }{  VI.2      }{  representatives of orbits of simple roots }
\lish{}{}{\quad under $\Gal$}
\blank
\lish{$\epsilon$          }{  III.1     }{  positive root in a rank two group}
\lish{$\epsilon_{\pm\alpha}(x)$ }{      I.5   }{         $\exp(x\,X_{\pm\alpha})$, $X_{\pm\alpha}$ root vectors}
\lish{$\epsilon$          }{  II.7      }{  root $3\alpha+\beta$ of $G_2$}
\lish{$\epsilon$          }{  VII.5.5   }{  $\epsilon=1$ for $n$ even and $\epsilon=0$ for $n$ odd}
\lish{$\epsilon_j$        }{  VII.5.5   }{  permutation $(j,-j)$ of $-k,\ldots,k$}
\blank
\lish{$\zeta$             }{  III.1     }{  positive root in a rank two group}
\lish{$\zeta$             }{  II.7      }{  root $3\alpha+2\beta$ of $G_2$}
\blank
\lish{$\eta_K$            }{  V.4,VII.4       }{  nontrivial character of $H^1(\Gal,U(1))$}
\lish{$\eta_K$            }{       }{  nontrivial quadratic character of $F^\times$}
\blank
\lish{$\kappa$            }{  I.1,I.5       }{  character on $H^1(\Gal,T)$}
\lish{$\kappa^\alpha$     }{  VI.2      }{  character of $F_\alpha^\times$}
\lish{$\kappa(E)$         }{  V.2       }{  character such that $f/\kappa(E)(\mu)$ extends}
\lish{}{}{\quad generically to $E$}
\lish{$\kappa(E)$         }{  VI.2      }{  $\prod (\kappa^\alpha)^{e(\alpha)}$, $e(\alpha)$ an integral multiplicity}
\lish{$\kappa(E_0)$       }{  VI.2      }{  $=\kappa_0$}
\lish{$\kappa_0$          }{  VI.2      }{  $\prod \kappa^\alpha$}
\blank
\lish{$\lambda$           }{  I.1       }{  local parameter on $\Gamma$ at $p$}
\lish{$\lambda$           }{  II.4      }{  pullback of a local parameter on $\Gamma$ to $Y$}
\blank
\lish{$\nu$               }{  I.2,II.1       }{  an element of $N_\infty$}
\lish{$\nu_2$             }{  V.3       }{  form on $E_\alpha\cap E_\beta$}
\blank
\lish{$\xi$  }{  I.5,V.1.2  }{  coordinate in $N_{qs\infty}/N_\alpha$ on $U(\alpha,\beta)\cap E_\alpha(u)^0$}
\lish{$\xi$               }{  IV.5      }{  a wall of an $\alpha$-chamber}
\lish{$\xi:X_1\to T$      }{  I.6       }{  a morphism}
\lish{$\xi:{}^LH\to{}^LG$ }{            }{  an embedding of $L$-groups}
\blank
\lish{$\pi_1:X_1\to G$    }{  I.6       }{  a morphism}
\lish{$\pi_1:Y_1\to G$    }{  I.5       }{  a morphism}
\blank
\lish{$\rho$              }{  VII.1     }{  homomorphism $\Gal\to\hbox{Outer}({}^LH^0)$}
\blank
\lish{$\sigma_\alpha$     }{  I.5,II.6,V.5.2     }{  simple reflection in the Weyl group}
\lish{$\sigma_\omega$     }{  II.6      }{  representative of $\omega\in\Omega$ in $N_G(T_0)$}
\lish{$\sigma_*$          }{  VI.1      }{  action of Galois group on simple roots in}
\lish{}{}{\quad a quasisplit group}
\lish{$\sigma_{sp}$       }{  VI.1      }{  action of the Galois group in $G_{sp}$}
\lish{$\sigma_0$          }{  VI.1      }{  element of $\tilde \Omega$}
\lish{$\sigma_T$          }{  III.1     }{  permutation of Weyl chambers associated }
\lish{}{}{\quad to $\sigma\in\Gal$ and $T$}
\lish{$\Sigma$            }{  VI.2      }{  an element of $\Delta'$}
\blank
\lish{$\phi:T^0\times T\back G\to X_1$      }{  I.6          }{      a morphism}
\blank
\lish{$\omega_\alpha$     }{  V.5.2     }{  $\omega_\alpha\in N_G(T_0)$}
\lish{$\omega_\sigma$     }{  VI.1      }{  element of $N_{P_\alpha}(T_0)_{adj}$}
\lish{$\omega_Z$          }{  I.2,V.2   }{  invariant form of top degree on $Z$,}
\lish{}{}{\quad $Z=M,T,T\back G,X$ or $Y$}
\lish{$\Omega$            }{  I.2       }{  Weyl group of $G$ with respect to $T$}
\lish{$\tilde\Omega$      }{  VI.1      }{  the extended Weyl group or direct }
\lish{}{}{\quad product $\Omega\times \ZZ/2\ZZ$}
\lish{$\Omega'$           }{  III.1     }{  $\hbox{Image}(\Gal\to\tilde\Omega)$, $\tilde\Omega$ extended }
\lish{}{}{\quad Weyl group}
\blank
\lish{adjacent walls      }{  IV.4      }{}
\lish{adjoint conjugacy   }{  I.6       }{  a conjugacy class in the adjoint group}
\lish{$a_\sigma$          }{  V.4       }{  cocycle in $H^1(\Gal,U(1))$}
\lish{$a_\sigma$          }{  VI.1      }{  automorphism of $(G_{sp},B_{sp},T_{sp},\{X_\gamma\})$}
\lish{$a_\sigma(w)$       }{  VI.2.3    }{  a cocycle}
\lish{$a_n$               }{  VII.5.5   }{  the cocycle $a$ for the group $U(n)$}
\lish{$a(E_\Sigma)$       }{  II.9      }{  Igusa constants}
\lish{$A_\sigma$          }{  I.5       }{  a cocycle of $Z^1(\Gal,N_{G_{qs}}(T_{qs})_{ad})$}
\lish{$A(X),A_r(X)$       }{  V.3       }{  Igusa data, see Langlands [\LL]}
\lish{$A_n$               }{  I.3       }{  group or algebra of type $A_n$}
\lish{$\AAA^r$             }{            }{  affine $r$-space}
\blank
\lish{big nodes           }{  IV.4      }{}
\lish{big chamber         }{  IV.4         }{      }
\lish{big wall            }{  IV.4      }{}
\lish{$b$                 }{  II.1      }{  element of $B_0$}
\lish{${\bold b}=b_{n,2h}$  }{  VII.5.5   }{  a cocycle in $H^1(U(1))$}
\lish{$\BB$               }{  I.2       }{  Borel subgroup containing $T$}
\lish{$\BB(W)$            }{  I.2       }{  $\BB^\omega$, $W = W(\omega)$.}
\lish{$(B(W))$            }{  I.2       }{  star in $S$}
\lish{$B_0,B_\infty$      }{  I.2       }{  a pair of opposite Borel subgroups}
\lish{$B_{qs}$            }{  I.5       }{  Borel subgroup over $F$ containing $T_{qs}$}
\lish{$B_{sp}$            }{  VI.1      }{  Borel subgroup in split form}
\lish{$B_+$               }{  V.5       }{  intersection of lines $\ell_\alpha$, $\ell_\beta$}
\lish{$B_-$               }{  V.5.1     }{  intersection of lines $\ell_\alpha$, $\ell_\alpha'$}
\lish{B.I,$\ldots$,B.IV }{  III.1     }{  zero patterns}
\lish{$B_n$               }{  I.3       }{  group or algebra of type $B_n$}
\blank
\lish{$\CC$               }{            }{  complex numbers}
\lish{$C_G(x)$            }{            }{  centralizer in $G$ of $x$}
\lish{$C_G(x)_{red}$      }{  VII.5     }{  the reductive part of the centralizer of $x$}
\lish{$C_n$               }{  I.3       }{  group or algebra of type $C_n$}
\blank
\lish{divisor             }{  I.6       }{}
\lish{--\ fundamental }{  II.9      }{}
\lish{--\ $O$         }{  I.6       }{  a divisor over the unipotent class $O$}
\lish{--\  regular   }{  I.6       }{  a divisor over a regular unipotent class}
\lish{--\  spurious    }{  II.9      }{}
\lish{--\ subregular}{  I.6       }{  a divisor over a subregular unipotent class}
\lish{$D_n$               }{  I.3       }{  group or algebra of type $D_n$}
\lish{Dynkin curve        }{  IV.1,VI.1      }{  $(B\back G)_u$}
\blank
\lish{external wall       }{  IV.5      }{   }
\lish{$e$                 }{  I.5,II.1       }{  a star, element of $S$}
\lish{$e(p)$              }{  VI.2      }{  a star depending on a point $p$}
\lish{$e=e(\alpha_1,\alpha_2)$  }{      V.6   }{         constant defined by }
\lish{}{}{\quad $\exp(X_{-\alpha_1})\exp(X_{\alpha_1+\alpha_2})\exp(-X_{-\alpha_1})=\ldots$}
\lish{$e_\ell$            }{  VII.5.6   }{  constants in $F$}
\lish{$E$                 }{  I.6       }{  a divisor}
\lish{$E(u)$              }{  I.6       }{  the fibre in $E$ over $u\in G$}
\lish{$E_\alpha$          }{  V.1       }{  a subregular divisor, $\alpha$-simple, $E_\alpha = E_\Sigma$,}
\lish{}{}{\quad $\Sigma=\{\alpha\}$}
\lish{$E_\alpha(u)$       }{  V.1.1     }{  $\pi^{-1}(u)\cap E_\alpha$, fibre over $u$ in $E_\alpha$}
\lish{$E_\alpha(u)^0$     }{  V.1.1     }{  an irreducible component of $E_\alpha(u)$}
\lish{$E(\ell_\alpha,u)$  }{  V.1.2     }{  a component of $E_\alpha(u)$ indexed by the line $\ell_\alpha$}
\lish{$E_0$               }{  V.1       }{  the regular divisor, $E_0=E_\Sigma$, $\Sigma=\emptyset$}
\lish{$E$                 }{  VII.5.14  }{  $E_{\alpha_k}\cap E_{\alpha_{k+1}}$ or $E_{\alpha_k}$}
\lish{$E_n$               }{  I.3       }{  group or algebra of type $E_n$, $n=6,7,8$}
\blank
\lish{fundamental}{}{}
\lish{--\ $\alpha$-cell }{      IV.5  }{          }
\lish{--\ divisor }{  II.9      }{   }
\lish{$f$                 }{  I.5       }{  a locally constant fuunction of compact }
\lish{}{}{\quad support on $G$}
\lish{$F$                 }{  I.        }{  $p$-adic field of characteristic 0.}
\lish{$\Fb$               }{  I.        }{  algebraic closure of $F$}
\lish{$F_\alpha$          }{  VI.2      }{  field extension of $F$ fixing $\alpha$}
\lish{$F_4$               }{  I.3       }{  group or algebra of type $F_4$}
\lish{$F_1(\beta,\theta,f)$     }{      V.3   }{ a term of the asymptotic expansion,}
\lish{}{}{\quad see Langlands [\LL]}
\blank
\lish{$G$                 }{  I.        }{  reductive group defined over $F$}
\lish{$G_{adj}$           }{  I.6       }{  the adjoint group of $G$}
\lish{$G_{der}$           }{  II.5      }{  the derived group of $G$}
\lish{$G_{in}$            }{  VII.4     }{  inner form of $G$}
\lish{$G_{in}$            }{  I.5       }{  an inner form of $G_{qs}$}
\lish{$G_{qs}$            }{  I.5       }{  a quasi-split group, a form of $G$}
\lish{$G_{sp}$            }{  VI.1      }{  split form of $G$}
\lish{$G_2$               }{  I.3       }{  group or algebra of type $G_2$}
\blank
\lish{$h_2$               }{  V.3       }{  function on $E_\alpha\cap E_\beta$}
\lish{$H^1(T)$            }{  I.5       }{  $H^1(T)=H^1(\Gal,T)$ first }
\lish{}{}{\quad cohomology group with coefficients in $T$}
\lish{$H$                 }{  I.5,VII.1  }{  endoscopic group}
\lish{$H$                 }{  VII.5     }{  the endoscopic group $U(n-2h)\times U(2h)$ }
\lish{}{}{\quad of $U(n)$}
\lish{$\underline{H}$     }{  VII.5.11  }{  subgroup $U(2h)\times U(n-2h)$ of $G=U(n)$}
\lish{$H'$                }{  VII.5.11  }{  subgroup of $U(n)$ conjugate to $\underline{H}$}
\blank
\lish{$K$}{}{ field extension of $F$}
\lish{$K_X$               }{  V.2.3     }{  field of rational functions on $Lie(T)$}
\lish{$K_\alpha$          }{  IV.7      }{  an $\alpha$-cell in $Z_\alpha^-$}
\blank
\lish{$\ell_\alpha$       }{  V.1.2     }{  a projective line of type $\alpha$ in }
\lish{}{}{\quad the flag variety}
\lish{$L_p(W)$            }{  IV.1      }{  a set of simple roots}
\lish{$L^- = L^-_{\beta_i}$     }{      IV.5,IV.7  }{         }
\blank
\lish{$m_\alpha$          }{  II.2      }{  $\beta = \sum m(\alpha)\alpha$}
\lish{$m_\kappa(e)$       }{  I.5       }{  the function $g\to\kappa(\sigma(g)g^{-1})$,}
\lish{}{}{\quad $\sigma\in \Gal$, $g\in (T\backslash G)(F)$}
\lish{$M$                 }{  I.2       }{  Springer variety $B\times_B G$}
\lish{$M$                 }{  V.3       }{  constant $m+m_1+m_2+am(\mu_3)$}
\blank
\lish{node                }{  I.3       }{  a node is an $\{i,j\}$-residue in the Coxeter }
\lish{}{}{\quad complex of $\Omega$}
\lish{--\ big            }{  IV.4      }{   }
\lish{--\ solid           }{  IV.3      }{   }
\lish{--\ special        }{  III.1     }{   }
\lish{$n_w$               }{  II.1      }{  a coordinate in $N_\infty$: $B_0^{n_w} = B(W)$}
\lish{$N_G(T)$            }{            }{  normalizer in $G$ of $T$}
\lish{$N$                 }{  I.2       }{  unipotent radical of $\BB$}
\lish{$N_\infty$          }{  I.2       }{  the unipotent radical of $B_\infty$}
\lish{$N_{qs}$            }{  I.5       }{  unipotent radical of $B_{qs}$}
\lish{$N_{qs\infty}$      }{  I.5       }{  unipotent radical of Borel opposite $B_{qs}$ }
\lish{}{}{\quad through $T_{qs}$}
\lish{$N_\alpha$          }{  I.5       }{  radical of the parabolic subgroup }
\lish{}{}{\quad $B^{op}_{qs}\langle \sigma_\alpha\rangle B_{qs}^{op}$, $op=$ opposite}
\lish{$N_\omega$          }{  II.6      }{  subgroup of $N_0$}
\lish{$N$                 }{  VI.2.3    }{  the norm map from $K$ to $F$, $K$ a field}
\blank
\blank
\lish{obtusely adjacent   }{  IV.4      }{   }
\lish{$O$-divisor         }{  I.6       }{  a divisor over the unipotent class $O$}
\lish{$O$                 }{  II.9      }{  unipotent class $O$}
\blank
\blank
\lish{proximate chamber   }{  IV.4      }{   }
\lish{$p$                 }{  I.1       }{  point on a curve $\Gamma$}
\lish{$p$                 }{  II.1      }{  a point in $X_1$}
\lish{$p$                 }{  I.        }{  residue characteristic of $F$}
\lish{$\PP_\alpha$        }{  II.6      }{  parabolic subgroup containing $B_0$}
\lish{$P_\alpha$          }{  IV.1,V.1.2     }{  a parabolic subgroup of type $\alpha$ }
\lish{}{}{\quad containing $B_0$}
\lish{$\PP^1$             }{  VII.4     }{  projective line}
\blank
\blank
\blank
\lish{$Q$-chamber         }{  IV.5      }{   }
\blank
\lish{regular divisor     }{  I.6       }{  a divisor over a regular unipotent class}
\lish{$r$                 }{  I.2       }{  the semisimple rank of $G$}
\lish{$R$                 }{  IV.3      }{  roots $\alpha$ such that $z(W,\alpha)\ne 0$ for some $W$}
\lish{$R_\omega$          }{  II.7      }{  $\{\beta>0 | \omega\beta<0\}$}
\lish{$R_1,R_2$           }{  VII.5.5   }{  elements of $K_X$}
\blank
\blank
\lish{solid node          }{  IV.3      }{   }
\lish{special node        }{  III.1     }{   }
\lish{spurious divisor    }{  II.9      }{   }
\lish{subregular}{}{}
\lish{--\ divisor  }{  I.6       }{  a divisor over a subregular unipotent class}
\lish{--\ conjugacy class }{  IV.1         }{       }
\lish{--\ unipotent  }{      III.3 }{          }
\lish{$S$                 }{  I.2       }{  variety of stars}
\lish{$S'$                }{  I.2       }{  subvariety of $S$}
\lish{$S''$               }{  I.2       }{  open subvariety of $S_1$}
\lish{$S^0$               }{  I.2       }{  variety of regular stars}
\lish{$S_1$               }{  I.2       }{  first resolution of $S$}
\lish{$S(B_\infty)$       }{  I.2       }{  a coordinate patch on $S$}
\lish{$S(B_\infty,B_0)$   }{  I.2       }{  a coordinate patch}
\lish{$S_2$               }{  IV.4      }{   }
\lish{$S$                 }{  IV.3      }{  roots $\alpha$ such that $z_1(W,\alpha)=0$ for some $W$}
\lish{$S_-$               }{  IV.4      }{  $S_- = S$ except for $D_4,\ |S|\ge 4,\ldots$}
\blank
\lish{type of a vertex    }{  IV.3.2    }{   }
\lish{$t_\sigma$          }{  I.5       }{  a cocycle on $T(R)$, $R$ the field of rational }
\lish{}{}{\quad functions on $S$}
\lish{$T$                 }{  I.2       }{  Cartan subgroup in $G$ over $F$}
\lish{$T^0$               }{  I.2       }{  regular elements of $T$}
\lish{$T_0$               }{  I.2       }{  intersection of opposite Borel }
\lish{}{}{\quad subgroups $B_0$, $B_\infty$}
\lish{$T_0$               }{  V.5.2     }{  Cartan subgroup in $B_+\cap B_-$}
\lish{$T_{in}$            }{  I.5       }{  Cartan subgroup in $G_{in}$}
\lish{$T_{qs}$            }{  I.5       }{  maximally split Cartan subgroup of $G_{qs}$}
\lish{$T_{sp}$            }{  VI.1      }{  Cartan subgroup in split form}
\lish{$T(W,\alpha)$       }{  IV.1      }{  $(1-\gamma^{-1})/\lambda$}
\lish{$T_\sigma$          }{  I.5       }{  a twisted cocycle in $Z^1(T_{qs})$}
\lish{$T_i$               }{  VII.5.5   }{  the $i$th character on the Cartan subalgebra}
\blank
\blank
\lish{$U(1)$              }{  V.4       }{  unitary group in 1-variable}
\lish{$U(\alpha',\alpha'')$     }{      V.1.1 }{         a coordinate patch in $Y_s$}
\lish{$U_1,U_2$           }{  V.3       }{  coordinate patches}
\blank
\lish{$V^n$               }{  I.2       }{  $n=|\Omega|$-fold product of the flag variety}
\blank
\lish{$w=w(\alpha+\beta)$ }{  V.2       }{  a coordinate on $U(\alpha,\beta)\cap E_\alpha(u)^0$}
\lish{$w_n$               }{  VII.5.5   }{  the variable $w$ for the group $U(n)$}
\lish{$W_+$               }{  I.2       }{  positive Weyl chamber for $\BB$}
\lish{$W(\omega)$         }{  I.2       }{  $\omega^{-1}W_+$, $\omega\in\Omega$}
\lish{$\bWW$               }{  IV.4      }{  a big chamber}
\lish{$\bW$               }{            }{  a $Q$-chamber}
\blank
\lish{$x_\beta$           }{  II.1      }{  $x_\beta(b)$ is the $\beta$th coefficient of $b\in B_0$}
\lish{$x(W,\beta)$        }{  II.1      }{  a local coordinate}
\lish{$(x_{-k},\ldots,x_{k})$   }{      VII.5.5            }{      element of a Cartan subalgebra for }
\lish{}{}{\quad $U(n)$, $2k+1-\epsilon=n$}
\lish{$X$                 }{  I.2       }{  closure of $X^0$ in $G\times S$}
\lish{$X'$                }{  I.2       }{  closure of $X^0$ in $G\times S'$}
\lish{$X''$               }{  I.2       }{  closure of $X^0$ in $G \times S''$}
\lish{$X(B_\infty,B_0)$   }{  I.2       }{  a coordinate patch on $X$}
\lish{$X^0$               }{  I.2       }{  subvariety of $G\times S^0$}
\lish{$X_1$               }{  I.2       }{  closure of $X^0$ in $G \times S_1$}
\lish{$X_{\pm\alpha}$          }{  I.2,VI.2      }{  root vectors}
\lish{$X_{\pm\alpha},H_\alpha$  }{      II.1  }{         a Lie triple}
\lish{$X(H)$              }{  VII.3     }{  a set of characters $\theta:F^\times\to\CC^\times$}
\blank
\blank
\lish{$y_\beta$           }{  II.2      }{  $\beta$th coefficient of $t^n$}
\lish{$Y$                 }{  I.2       }{  closure of $Y^0$ in $X$}
\lish{$Y'$                }{  I.2       }{  closure of $Y^0$ in $X'$}
\lish{$Y''$               }{  I.2       }{  closure of $Y^0$ in $X''$}
\lish{$Y(B_\infty,B_0)$   }{  I.2       }{  a coordinate patch on $Y$}
\lish{$Y_\Gamma$          }{  I.2       }{  resolution of singularities of $Y_1$}
\lish{$Y_\Gamma$          }{  I.6       }{  resolution of $Y_1$}
\lish{$Y^0$               }{  I.2       }{  restriction of $X^0$ to $\Gamma$}
\lish{$Y_1$               }{  I.2       }{  closure of $Y^0$ in $X_1$}
\lish{$Y_s$               }{  V.1       }{  an open subvariety of $Y''$}
\lish{$Y(T,\kappa)$       }{  VII.3     }{  a set of characters $\theta:F^\times\CC^\times$}
\blank
\lish{zero pattern        }{  III.1     }{   }
\lish{$\hat z(\alpha)$     }{  IV.4      }{  a variable in $S_2$}
\lish{$\hat z(W,\alpha)$   }{  IV.4      }{  a variable in $S_2$}
\lish{$\tilde z(W,\alpha)$      }{      IV.1  }{         $\tilde z(W,\alpha) = z(W,\alpha)/z(W_\alpha,\alpha)$}
\lish{$z(W,\alpha)$       }{  I.2       }{  a coordinate on $S(B_\infty,B_0)$}
\lish{$z_1(W,\alpha)$     }{  I.2       }{  homogeneous coordinate in $S_1(B_\infty,B_0)$}
\lish{$z_1(W,\alpha)$     }{  II.2      }{  $z_1(W,\alpha)=z(W,\alpha)/z(\alpha)$}
\lish{$\ZZ$               }{            }{  ring of integers}
\lish{$Z$                 }{  VII.5     }{  center of $G$}
\lish{$Z_\alpha^-$        }{  IV.7      }{  union of $\alpha$-chambers}
\lish{$Z_\beta$           }{  IV.7      }{  union of $\beta$-chambers}
