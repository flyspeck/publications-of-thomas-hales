...
$\VF$-points are
\[
t =  \phi_\sigma(\sigma(t))^{w_\sigma}, \quad\forall \sigma\in\op{Gal}(K/\VF),
\]
...
the field $F_1$ has characteristic zero~\cite{W}.
...
\[
B^r = B^r_{kk'\s \s'} = \{(b_1,\ldots,b_r) \mid b_i \in B^1_{k_ik'_is_it_i}\quad
...
P^r_{kk'\s \s'} = P^r = \{(X_H,b) \mid b = (b_1,\ldots,b_r)\in B^r_{kk'\s \s'},\quad
  X_H = (X_H^1,\ldots,X_H^r), \quad (X_H^i,b_i) \in P^1_{k_ik'_is_it_i}\}.
\]
...
P^r_{kk'\s \s'}$,  $\rho=1,\ldots,r$, and $i_\rho\in J_\rho(k,k')$,
set
\[
\psi^{\rho,i_\rho}(X_H^\rho,b_\rho) = 
\begin{cases}   
  O^\kappa(X_H^\rho,X_G(b_\rho),\ldots,1_{\Y_G^{i'_\rho}(b_\rho)}),
   & \text{if } i_\rho = (i'_\rho,-)\in J^-_\rho(k) \\
  O^{st}(X_H^\rho,1_{\Y_H^{i'_\rho}}),
   & \text{if } i_\rho = (i'_\rho,+)\in J^+_\rho(k').
\end{cases}
\]
 We also define constructible functions
on $P_{kk'\s \s'}$ indexed by $i\in J(k,k')$:
\[
(X_H,b)\mapsto \theta^i(X_H)_b := \prod_{\rho=1}^r \psi^{\rho,i_\rho}(X_H^\rho,b_\rho).
\]
...
$k_\rho$ is the number of nilpotent orbits in $\fg_{z_\rho,F_2}$.  By the
...
$\s_\rho\in \cF_G^{k_\rho}$ such that $\NF^{k_\rho}_{\s_\rho,F_2}\ne
\emptyset$.  Let $\s = (\s_1,\ldots,\s_r)$.

Let $k'_\rho$ be the dimension of the space of stable Shalika germs on
...
the unipotent set.  For each $\rho$, let $\mu_i^{st}$, for $i\le k'_\rho$,
...
parameter $\Y^\rho_{H_{z_\rho},F}\in \NF^{k'_\rho}_{H_{z_\rho}}(F)$ such
that the functions $1_{\Y_H^{\rho,j}}$, for $j=1,\ldots,k'_\rho$, form a
...
do this by Theorem~\ref{thm:bm}.  Let $\s'_\rho = \s(\Y^\rho_{H_{z_\rho},F})\in
S^{k'_\rho}_{H}$ and $\s' = (\s'_1,\ldots,\s'_r)$.  We have now fixed the
...
$1_{a_0,F_1}\theta^i_{b,F_1}$, for $i\in \{j\}\cup J^+$, are
$a_0$-asymptotically linearly dependent.  If there exists $a_0$ such
that for all $b\in B^r(F_1)$, the functions indexed by $J^+$ are
$a_0$-asymptotically linearly dependent, this is immediate.  If, on
the other hand, for all $a_0$, there exists $b$ such that the
functions indexed by $J^+$ are not $a_0$-asymptotically linearly
dependent, then there exists a parameter $\Y_H(b)$ from the base space
$B^r(F_1)$ such that for each $\rho$, the functions
$1_{\Y_{H,b}^{\rho,j}}$ give a dual basis to the stable distributions
on the nilpotent set of $\fh_{z_\rho}$.  We use Waldspurger's
fundamental result \cite{W} that the $\kappa$-orbital integral (over
$F_1$) admits smooth matching, which implies that there exists $a_0$
such that $1_{a_0}\theta^{j}_{b,F_1}$ is a linear combination of
$1_{a_0}\theta^i_{b,F_1}$, for $i\in J^+$.  This proves the claim.


We apply Theorem~\ref{thm:cgh-asymp} to the morphism $f:P^r_{kk'\s \s'}\times\ring{Z}\to
B^r_{kk'\s \s'}$.  By the theorem, for each $j\in J\setminus J^+$, and for
all $b'\in B^r_{F_2}$, the functions $\theta^i_{b',F_2}$, for $i\in
\{j\}\cup J^+$ are $a_0$-asymptotically linearly dependent.  Let $c^i_{j,b'}\in \ring{C}$
...
\sum_{i\in \{j\}\cup J^+} c^i_{j,b'}1_a\theta^i_{b',F_2}  =
...
becomes
\begin{equation}\label{eqn:du}
\sum_{i\in J^+} c^i_{j,b'}1_a\theta^i_{b',F_2}  = 0.
\end{equation}
This asserts that for all $b'\in B^r(F_2)$, the functions
$1_a\theta^i_{b',F_2}$ are linearly dependent.  Applying
...
$b\in B^r(F_1)$, the functions $1_a\theta^i_{b,F_1}$, for $i\in J^+$ are linearly
...
involving $1_a\theta^j_{b',F_2}$ as a function of $X_H^\rho$ alone, it
...
assumptions, a family of $p$-adic integrals with parameter $x$
