...
We may replace the components $f_i$ of $\bf$ with $f_i 1_P$ on $P'\times\ring{Z}$ to reduce
...
products of Shalika germs.
The second statement may be used to extract a nontrivial linear relation on
a factor from a linear relation on the tensor product.
...
Recall that $m$ is the natural number that we use as a lower bound in
Theorem~\ref{thm:cgh-asymp} on the residual characteristic in $\Loc_m$
.  By invoking the theorem a finite number of times, we may pick a
single natural numbers $m$ that works in the theorem in all cases by
taking the maximum of the natural numbers obtained in the finitely
many separate cases.  Let $m$ be a natural number that works for all
$k,k',\s,\s'$ (keeping the other choices fixed).
...
$z(\lambda_i)$, $X_G(\lambda_i)$, $\Y_G(\lambda_i)$, and so forth for the components of $\lambda_i\in\Lambda^1$.  
If $j\in\ring{N}$ and $\sigma\in\{\pm\}$, write
\[
\Y(\lambda_i,(j,\sigma)) = 
\begin{cases}
\Y_G(\lambda_i)_j,&\text{if }\sigma= -\\
\Y_H(\lambda_i)_j,&\text{if }\sigma= +.
\end{cases}
\]
We
...
For any $r$-tuples  $k=(k_1,\ldots,k_r)$ and $k'=(k'_1,\ldots,k'_r)$, set
...
We now give the proof of Theorem~\ref{thm:local}

\begin{proof}
  To lighten notation, we sometimes drop $r,k,k',\s,\s'$.  In
  particular, we set $L=L(k,k')$, $L^+=L^+(k')$, $\Lambda^r =
  \Lambda^r_{kk'\s\s'}$, $P^r = P^r_{kk'\s\s'}$.
