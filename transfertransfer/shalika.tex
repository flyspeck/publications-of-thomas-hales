...
the paper.  Our first theorem establishes the existence of 
constructible motivic functions that represent Shalika germs.

We use the concepts of {\it definable sets} and {\it constructible
  motivic functions} (or constructible functions for short) from
\cite{CL}.  All constructible functions in this paper are understood
...
of $G$.  We give a constructible function $1_\Y$ parametrized
...
elements in $\fg$.  The following theorem is proved in
Section~\ref{sec:msg}.
...
\begin{theorem} [Lie algebra Shalika germs]\label{thm:lie-shalika} 
  ...
  orbits in $\fg_z(F)$, then $d_k$ does not vanish anywhere on the
...
can be represented as fiber $G_z$ in a definable reductive group $G$,
...
$n$-tuples of pairwise non-conjugate Barbasch-Moy unipotent pairs in
$G$.  We give a constructible function $1_\Y$ parametrized by $\Y\in
\UF^1_G$.  The following theorem is proved in Section~\ref{sec:adapt}.
...
theory of multiplicative characters for motivic integration. The following
theorem is proved in Section~\ref{sec:adapt}, by adapting the Lie algebra
proof.
...
$\tilde H(F)$ \cite{LSxf}*{\S 4.4}.  The following theorem  is the main
reesult of the paper.  The proof appears in Section~\ref{sec:xfer}.
...
Galois groups isomorphic to $\Sigma$ given by tuples
...
\begin{proof} All the implications mentioned in this proof should be
  ...
  cite. The equivalence of the first two properties is well-known
...
\begin{theorem}\label{thm:bmg} Let $G_{/Z}$ be as above.  There exists $m$ such that
  for all $F\in \op{Loc}_m$, $z\in Z(F)$, and for all unipotent
  conjugacy classes in $G_z(F)$, there exists $\s\in \cF$ and an
  element $u$ in the unipotent conjugacy class such that
\begin{enumerate}
...
\begin{proof} First, we show that (1) and (2) are first-order
 ...
 J. Adler \cite{adler:anisotropic}*{\S 1.5-1.6}.  When the exponential
...
form in (\ref{eqn:omegaK}), we see that the Galois group preserves the
...
(\ref{eqn:xfer-domain}).  The definable set $D_{j,\rom{1}}$ is a
subset of $\fh^{G-\reg}\times_Z\fg^\reg$, which we pull back to sets
$D^{12}_{j,\rom1}$ and $D^{34}_{j,\rom1}$ on (\ref{eqn:xfer-domain})
under the projections maps onto the first two factors, and the last
two factors of (\ref{eqn:xfer-domain}), respectively:
\[
...
the number $\ell$ of terms in the sum.  This is \cite[Lemma
...
unbounded Presburger sets in the disjoint union.
...
\begin{theorem}[Cluckers-Gordon-Halupczok]\label{thm:cgh}
  ...
  functions $\bf_{\lambda,F_1}$ is linearly dependent, then then also
...
Shalika germs are indexed by nilpotent orbits and the number $k$ of
...
field, $k$ is larger than the actual number of nilpotent orbits, a set
...
$N_1,\ldots,N_k$ are pairwise non-conjugate.  

%For any $\Y\in \NF^k$,
%we let $\s(\Y)=(\s_1,\ldots,\s_k)$ denote the tuple of second
%coordinates of $\Y$.  

For any $k$-tuple $\s=(\s_1,\ldots,\s_k)$, we write $\NF^k_\s$ for the
subset of $\NF^k$ consisting of those elements whose $k$-tuple of second
coordinates is $\s$.  For any $\Y=(N,\s)\in \NF^1$, let $1_\Y$ be the
...
definition of adjugate that orbital integrals have a constructible Shalika
germ expansion
\[
d_k(\Y) O(X,\Y) = \Theta(\Y)\Gamma(X,\Y),
\]
That is, up to a (nonzero) determinant $d_k$, the orbital
...
then it follows from Theorem~\ref{thm:bm} that $\NF^k(F)$ is nonempty,
and for all $\Y\in \NF^k(F)$, we have $\det\Theta_F(\Y)$ is
...
most intricate proof in the paper.  It will occupy
...
measures $d\mu^\reg_G$ and $d\mu^\reg_H$ on the stable orbits of $X_G$
and $X_H$.
...
\end{equation}
where $f$ is a constructible function on $\fg$ and the
...
\Y_G\in \NF^{k_i}_{G_z,\s_i}, \Y_H\in \NF^{k'_i}_{H_z,\s'_i}\}.
\end{align*}
...
\end{cases}
\end{equation}
We also define a tuple $\bf=(\ldots,f_\ell,\ldots)$ of constructible
...
the theorem.  We let $m$ be the maximum of the constants $m(\xi)$ as
...
$\ell\in \{j\}\cup L^+$, is $a_0$-asymptotically linearly dependent.
...
$f:P^r\times\ring{Z}\to \Lambda^r$.  By the theorem, for each $j\in
L\setminus L^+$, and for all $\lambda_2\in \Lambda^r({F_2})$, the
...
We claim that there exists $\lambda_2\in \Lambda^r({F_2})$, such that
for all $j\in L\setminus L^+$, $c_{j,j,\lambda_2}\ne 0$.  Otherwise,
for all $\lambda_2\in \Lambda^r(F_2)$, there exists $j\in L\setminus
L^+$ such that $c_{j,j,\lambda_2}=0$.  Equation~\ref{eqn:du} becomes
...
$1_af_{\ell,\lambda_2,F_2}$ are linearly dependent for $a\ge a_0$.
Applying Theorem~\ref{thm:cgh-asymp} again in the opposite direction,
to go from dependence on $F_2$ to linear dependence on $F_1$, we find
that for all $\lambda\in \Lambda^r(F_1)$, the functions
$1_af_{\ell,\lambda,F_1}$, for $\ell\in L^+$ are linearly dependent.
The choice of $k',\s'$ gives linear independence, contrary to the
assumption of dependence.  This gives the claim.
...
nonvanishing $c_{j,j,\lambda_2}\ne 0$, gives for each $j\in L\setminus
L^+$ a linear relation for $1_af_{j,\lambda_2,F_2}$ in terms of
products of stable orbital integrals on factors $\fh_{z_i}$.  In
particular, for each $i\in\{1,\ldots,r\}$, and for each
$x\in\{1,\ldots,k_i\}$, we define $j = j(i,x) = (j_1,\ldots,j_r)$,
where
\[
...
linear relation involving $1_af_{j,\lambda_2,F_2}$ as a function of
$X_{H,i}$ alone, it gives a nontrivial linear relation between
functions $X_{H,i}\mapsto\psi_{i,x}(X_{H,i})_{\lambda_2}$ (with
nonzero coefficient) and the stable nilpotent orbital integrals on
$\fh_{z_i}$.  Lemma~\ref{thm:tensor} extracts the linear relation on a
factor from a linear relation on the tensor product.  As we run over
...
The Kostant section is replaced with the Steinberg
...
\begin{proof}[Proof of Theorem \ref{thm:local} for groups] 
  ...
  Barbasch-Moy pairs.  We have avoided properties of Shalika germ
  expansions (such as exponential maps and explicit estimates on the
  neighborhoods on which homogeneity holds) that do not obviously hold
  in the group.
\end{proof}
...
local fields in sufficiently large residual characteristic.

We recall some results from \cite{LSd}.  That article assumes that $F$
is a local field of characteristic zero.  We need to relax the
...
characteristic enters into the paper \cite{LSd} and how it can be
relaxed.  The assumption of characteristic zero is used in the parts
of the paper devoted to the archimedean places.  These sections of the
paper do not concern us here.  The assumption is used in an essential
...
calculations, which form the bulk of the paper, do not require the
assumption that the field has characteristic zero.

That article also cites other sources that assume that the
characteristic is zero.  We need Harish-Chandra descent of orbital
...
characteristic is proved by G. Prasad in
\cite{adler-debacker:mktheory}*{Appendix B}.

The article cites \cite{LSxf}, which also assumes the field has
characteristic zero. But here again, the arguments are readily adapted
to large characteristics.
