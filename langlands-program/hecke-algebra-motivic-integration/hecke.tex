...
Let $\hat S_G = \hat T/(1-\theta_G)\hat T$ and $\hat S_H = \hat T/(1-\theta_H)\hat T$.
...
%Choose $g\hat G$ such that $g \dotw  \theta_G g^{-1} = \theta_G$.  Adjusting $g$ by an element in $\hat G^{\theta_G}$, we may assume that
...
%$t_0$ and $\phi(\tilde S_H)$.  For this, we may work in $G_{adj}$ rather than $G_{sc}$.
...
%We compute that
%\[
%\lambda(t_0) = \zeta^{0\lambda_1+1\lambda_2+2\lambda_3+\cdots}.
%\]
...
\]
%We identify the root system of $E_7$ with the set of 
