...
We define a norm map on $\Phi^+$.  Let $\beta\in\Phi^+$, and let $B$ be its orbit under $\g{\theta}$.
...
%Assume the outer automorphism $\theta$ acts on $\hat G$ fixing a pinning including $\hat T$, $\hat B = \hat T \hat N$.  
%We now take $\hat G$ to be the
...
\begin{theorem}[Macdonald's formula]
...
%The automorphism $\theta$ acts as $\theta X_1 = X_2$, $\theta X_2 = X_1$, and $\theta X_3 = - X_3$, for $X_i\in \n_{\alpha_i}$.
\[
...
%We also wish to express the constants $q_\alpha$ and $q_{\alpha/2}$
...
\begin{proof}  \XX{add details of the proof.}  
...
The operator $J$ has the properties $J(f) = f J(1)$ if $f\in \ring{C}[X^*]^{W^\theta}$ and $J(\tau_\lambda) = J(e^\lambda)$.
