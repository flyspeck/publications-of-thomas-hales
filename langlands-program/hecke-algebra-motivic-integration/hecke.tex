...
\begin{proof}  Fix $\mu_0\in M$ and let $\mu_i = \theta_1^i \mu_0$.  The abelian group $\ang{\theta_1^b}$ acts on $V_{\mu_0}$.
...
\subsubsection{the partition function for the adjoint representation}
...
$P(E,q) = P(\hat G,\n,\theta,E,q)$.
%let $\n = \oplus_{\beta\in \Psi^+} \n_\beta$ be the root space decomposition.  
...
%$q^{\ang{\mu,\rho^\vee}} e^{\mu} P(E^{-1},1)/{Q_{S(\mu)}(q^{-1}) P(E^{-1},q^{-1})}$
...
%we have $\ang{\hat f_\lambda,\hat f_\mu} = c_\mu (\hat f_\lambda,e^\mu P(E,q^{-1})/P(E,1))$.
...
We have a simple set of roots $\Delta$, determined by $(\hat T,\hat B)$.  Let $\alpha$ be the highest
...
\end{equation}
%$g\in N_{\hat G}(\hat T)$.
%Choose $g\hat G$ such that $g \dotw  \theta g^{-1} = \theta$.  Adjusting $g$ by an element in $\hat G^{\theta}$, we may assume that
%$g$ preserves $\hat T$; that is, $g\in N_{\hat G}(\hat T)$.


\begin{lemma} If $(\hat G,\g_+,w\theta)$ has outer type with representative $\dotw\mapsto w$ satisfying Equation~\ref{eqn:outer}, then
\[
...
\]
%$(\hat G,\g_+,\theta_1)$ and $(\hat G,\g_+,\theta)$ have the same partition function.
Also, $\theta$ and $N_1$ preserve the set of positive roots, so that $\ang{v,N_1\mu}>0$ iff $\ang{v,\mu}>0$ iff $\mu\in \Psi^+$.
Hence $\g_+ = \g_{\Psi^+} = \n$.  The result follows.
\end{proof}


%We choose data $\epsilon=1$, $\tilde T_1 = \hat T_1 = \hat S$, and $\phi:\hat T_1\to \hat S$ the identity map.
...
\subsubsection{inner type and simply laced}  We will say that the data $(\hat G,\g_+,\theta_1)$ has inner type if $\theta=1$,
...
\begin{proof} We make a case-by-case calculation. 
...
%There is another observation that we use repeatedly.  The properties that we need to establish depend only on the roots of
%$\epsilon$ and $\phi(\tilde T_1)$.  For this, we may work in $G_{adj}$ rather than $G_{sc}$.
...
% we can take $\dotw$ to be a representative in the normalizer of a power of the Coxeter element in the Weyl group.
...
%Let $e_\gamma\in \n_\gamma\subset \op{gl}(n,\ring{C})$ be the root vectors with entries in $\{0,1\}$. Then again $\dotw e_\gamma = e_{w\gamma}$.
We have
\[
...
%Set
%$\phi:\tilde T_1 \to \hat T$, with $
%where $t_i = \tau_i I_{\ell}$.
%Let 
%\[
%\lambda = (\lambda_1,\lambda_2,\ldots,\lambda_n)\in Y^* = X^*(\hat T) = \ring{Z}^n.
%\]
%We compute that
%\[
%\lambda(\epsilon) = \zeta^{0\lambda_1+1\lambda_2+2\lambda_3+\cdots}.
%\]
Since $s$ is in diagonal form, $\op{Ad}(s) e_\gamma = \gamma(s) e_\gamma$.
Let $N_1\Psi$ be the root system of $\op{SL}(m,\ring{C})$. 
% (It can be identified with the  root
...
%\subsubsection{neither inner nor outer}

%This gives $\phi(s) \epsilon\in \hat S$.  Replacing $\phi(s) \epsilon$ with a Weyl-group conjugate if necessary, we arrange for it
%to satisfy the non-negativity property.  We observe that regularity property holds  in each case.

%We give data $\tilde T_1,\phi,\epsilon$ as well as the associated determinant $D_+$  in each case.
...
% (Note that we work in the adjoint group for convenience rather than the
%simply connected cover. \XX{justify this.})
...
An outer automorphism $\theta$ exchanges the two prongs of one of the forks.
...
%\begin{equation}
%D_{G,H} = \prod_{\alpha\in \Sigma^+} (1-e^\alpha) \prod_{\alpha\in \Sigma^+_{\text{long}}} (1-e^\alpha).
%\end{equation}
%Here $\Sigma$ is the root system of type $C_{n-2}$ with underlying torus $\hat T_1$ and positive roots $t_i t_j^{\pm 1}$, $t_i^2$.
%Also, $\Sigma_{\text{long}}$ is the subset of long roots in $\Sigma$.
...
%Let $\t$ be the Lie algebra of the complex torus $\hat T$, and let $X_*$ be the kernel of the exponential map from $\t$ to $\hat T$.
%Let $C$ be a fundamental chamber for the action of $X_*\rtimes W_G$ on $\t$.   Then we may choose the endoscopic data such
%that the image of $\dotw $ in $W_G$ is the image of an element in $X_*\rtimes W_G$ that
%preserves the chamber $C$.  
%This implies that we may choose $\dotw $ so that it acts as an automorphism of the extended Dynkin diagram
%of $\hat G$.


We state a theorem that extends some of the previous results to an arbitrary reductive group.
...
% where $s\mapsto t \mod (1-\theta_H)\hat T_H$;
...
%D_+(E_{G,H},1)$ is nonzero;
% and is divisible by  $D_H(E_H,1)$;
%\item (branching) Equation \ref{eqn:branch} gives the twisted branching rule for $\hat G\rtimes \theta$ restricted to $\xi(\hat H\rtimes \theta_H)$:
%\[
...
%We identify $X^*(\hat T_H)$ with a definable subset of $X^*(\hat T'_H)$ and identify $X^*(\hat T_G)$ with a definable subset of $X^*(\hat T'_G)$.
%To get the branching formula for $\tau_\lambda$, we pull it back to a representation of ${}^LG'$, restrict to $\xi'({}^LH')$, then restrict the parameters
%of $\sigma_\mu$ to
% $\mu\in X^*(\hat T_H)$.  
...
%Thus, we may assume that $\hat G$ has the form $\hat Z\times G_{sc}$.
%We have $X^*(\hat T) = X^*(\hat Z)\times X^*(\hat T_{sc})$, with a similar factorization for $\hat H' = \hat Z \times \hat H''$.
...
\XX{add calculation of $\epsilon,\phi$ for $E_n$.}
...
%(For our purposes, we want $m(\lambda,\mu)$ to be
...
%We will use an isogeny $\tilde T_1\mapsto \hat T_1$, to be constructed below.
%We will construct a homomorphism $\phi:\tilde T_1\to \hat S$ and an element of finite order $\epsilon\in \hat S$ such that
%$\xi(t\theta_H)$ is $\hat G$-conjugate to $\phi(s) \epsilon\theta\in \hat G\rtimes\theta$ , 
%whenever $s\in \tilde T_1$ and $t\in \hat T$ have the same image in $\hat T_1$.
...
\]
%By regularity, for all $\alpha\in M_{++}$, we have $N\alpha(\epsilon)\ne 0$.
...
Let $D(\hat H,\h_{\Psi^+_H},\theta_H,E_H,1)$ be the determinant attached to the set of positive roots in the adjoint representation $\h$ of  $\hat H$.
...
as in Theorem XX.
%, Then we can construct an isogeny $\tilde T_1\to \hat T_1$ of tori,
%a homomorphism $\phi:\tilde T_1\to \hat S$, and element $\epsilon\in \hat S$ of finite order, with
%associated determinant $D_{G,H}$ such that
%\begin{enumerate}
%\item (conjugacy) $\xi(t\theta_H)$ is $\hat G$-conjugate to $\phi(s) \epsilon\theta$,
%whenever $s\in \tilde T_1$ and $t\in \hat T$ have the same image in $\hat T_1$;
% where $s\mapsto t \mod (1-\theta_H)\hat T_H$;
%\item  (non-negativity) $\phi^*\alpha$ is non-negative for every positive
%root $\alpha$ of $\hat G$ (that is, if $\phi^*\alpha$ is nonzero, then it is positive);
...
In general, we write $X|$ for the restriction of $X$.
...
% that the first three properties imply the branching rule.
%Thus, the endoscopic branching formula comes down to constructing 
%the homomorphism $\phi:\tilde T_1\to \hat S$,
%the element $\epsilon\in \hat S$, 
...
%The group $\ang{\theta}$ acts on $W$ by
