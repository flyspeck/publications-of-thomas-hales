...
Hecke algebra $\H(G)=\H(G//K)$ of functions that are biinvariant with
...
The image of $\Phi$ in $X^*(A)$ is the restricted root system $\Sigma$.
...
\op{Sp}(2k,\ring{C})$ with root system $\Sigma_1$.  In this case, $G^1
...
%classes in $\hat H$ to $\theta$-conjugacy classes in $\hat G$.  
...
of both $\hat M_{sc}$ and $\hat G$.
  ...
  %Working with the simply connected cover, we allow twisted endoscopic groups with
  %non-trivial multiplicative character $\omega$ (and trivial automorphism) in
  %the sense of Kottwitz and Shelstad \cite{kottwitz1999foundations}.

  Let $\hat A$ be the maximal split torus in the center of $\hat M$.  Set
  \[
  ...
  Let $\hat B(w\theta)$ be the Borel containing $\hat T$ with positive roots
  ...
  Note that we can test whether $\phi^* N\alpha=0$ by evaluating on
   $\hat A^{\theta,0}$ by the map
  \[
  \hat A^{\theta,0} \to  \hat A_\theta \to \hat U \to \hat S.
  \]
  Also, $\alpha$ is trivial on $\hat A^{\theta,0}$ iff it is trivial on $\hat A$
  iff it is a root of $M$.  For all $\alpha\in \Psi_N$,
  \[
  ...
  We choose $\epsilon$ to be the image of $\epsilon_{sc}$ and $\dotw$ to be
  the image of $\dotw_{sc}$.
    ...
    is conjugate to $\iota_{sc}(\uu_{sc})\dotw\theta$ by an element $m\in\hat M_{sc}$
    whose image in $M$ commutes with $a\in Z(M)$.  Thus
    $a \epsilon_{sc}\phi_{sc}(\uu_{sc}) \theta$ and $\iota_{sc}(\uu_{sc})\dotw\theta$ are
    conjugate.
    ...
    \begin{align*}
      ...
      &=
      \det(1- \dotw \iota_{sc}(\uu_{sc}) \theta q;\g_N) \\
      &=
...
Let $\hat U = \hat U'$. %, $\phi = \phi'$ and $\epsilon=\epsilon'$.
Define $\iota$:
\[
...
%\det(1-\dotw_1^{-1} \theta\dotw_1 E q; \g_{\dotw_1^{-1}\Psi_{w'\theta}^+}) \\ &=
%\det(1 - \psi(\dotw_1^{-1} \theta\dotw_1 E) q; \g_{\Psi_{w'\theta}^+}) \\ &=
%\det(1- \theta \dotw_1 E \dotw_1^{-1} q;\g_{\Psi_{w'\theta}^+}) \\ &=
D(\hat G,\Psi_{w'\theta}^+,\dotw'\theta,{\dotw_1} (E),q).
\end{align*}
...
i^* D(\hat G,\Psi_{w'\theta}^+,\dotw'\theta,w_1(E),q) \\
&=i^{'*} (\op{Int} (w_1^{-1}))^* D(\hat G,\Psi_{w'\theta}^+,\dotw\theta,w_1(E),q) \\
&=i^{'*}  D(\hat G,\Psi_{w'\theta}^+,E,q) \\
&=\phi_\epsilon^{'*}  D(\hat G,\Psi_{\phi',\theta}^+,E,q) \\
&=\phi_\epsilon^{*}  D(\hat G,\Psi_{\phi,\theta}^+,E,q).
\end{align*}
...
U_{sc}$ by defining its character lattice to be the preimage in
$X_*(\hat U)$ of
\[
...
function as follows.  Let ${}^LH$ be an endoscopic group as above,
with $\theta_1 = \dotw \theta$.  Let $\hat B(\theta_1)$ and $\Psi^+_H$
be adapted.  We have a disjoint sum
\begin{equation}\label{eqn:disj-b1}
\Psi^+(\hat T,\hat B(\theta_1)) = 
\Psi^+_H \sqcup \Psi_{w\theta,0}^+ 
\sqcup \Psi_{w\theta}^+(\hat G\setminus\hat H),
\end{equation}
where $\Psi_{w\theta,0}^+$ is the set of $\hat B(\theta_1)$-positive
...
P^+_H\subseteq X^*(\hat T_1)$.  This section gives a branching rule
...
$\xi$.  Set $\theta_1 = \dotw\theta$.  We have a disjoint sum decomposition
\[
...
%\iota^* D(\hat G,\Psi_{w\theta}^+,\theta_1,E,1) \ \ 
...
%A {\it bound parameter} is similar, except that
...
\begin{definition} 
 ...
 \begin{lemma}  The set of strongly semisimple elements of an unramified
...
function of $L_G$.  It is a  $q$-constructible function on
$P^+\times G$.
...
\begin{proof} 
  The coefficients $s_{\lambda,\mu}$ are given by a integral of a $q$-constructible
...
\]
We ask whether $\op{vol}(T) = D(\theta,\ring{L})$ in the
...
The homomorphism $b_\xi(f_\lambda)$ can be replaced by
...
ranges over the definable subassignment of $a$-data.  The 
$a$-data are indexed by a fixed choice of indexing set.
...
%To check constructibility of $\chi_Z$, it is convenient
...
the embedding $\xi:{}^LH\to{}^LG$ (always assumed to factor through a
finite extension $\op{Gal}(E/F)$, the transfer factor changes by an
...
unique.  It be checked that the formulas for the transfer factor are
