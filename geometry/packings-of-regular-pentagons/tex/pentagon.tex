...
aligned pentagons pointing downward (Figure~\rif{fig:double-lattice}) \cite{Kup} \cite{henley}[p.801].  
...
(See Lemma~\rif{lemma:172}.) See Figure~\ref{fig:edge-172}.
\end{example}
...
\begin{lemma}[obtuse $b$]\libel{lemma:obtuse-b}  If $T$ is an obtuse Delaunay triangle, then $m_1(T)=m_2(T)=n_1(T)=0$.
...
\begin{proof} If $T_2$ is obtuse, then $m_2(T_2)=0$ by Lemma~\rif{lemma:obtuse-b}.
...
at the end of Section~\rif{sec:nonobtuse}.
...
\tikzfig{172-slider}{We can slide pentagons (that is, translate them along their common edge segment) to decrease
...
\begin{proof}
  ...
  $\kappa\sqrt8$.  By Lemma~\rif{lemma:primary}, we may assume
...
\begin{proof}
The shared edge has length less than $1.8$ by Lemma~\rif{calc:pseudo1}.  
It has length at least $1.72$ by Lemma~\rif{lemma:172}.
...
Let $\alpha = \alpha(T,e)$ be the angle between the two pentagons $A$ and $B$.  (See Figure~\rif{fig:good}.)  Modulo
...
This is easy to carry out.  By Lemmas~\rif{lemma:primary} and \ref{lemma:primary2}, we can move $B$ in an area decreasing way until $B$ has primary
...
\begin{proof} Suppose to the contrary that the rotation is increasing in $\dx{BC}$.  Choose coordinates so that the $x$-axis passes
...
We have classified all $P$-triangles with triple contact in Section~\rif{sec:contact}.  In this stage, we rely heavily
...
to labels on pentagons in the pin-$T$ configuration shown in Figure~\rif{fig:cord-pint}.  We claim that if the area the triangle has area less than
...
The triangles $T_0$ and $T_1$ separately have good coordinate systems described in Section~\rif{sec:coords}.  Three variables
...
A natural way to try to parameterize the dimer pairs of a given combinatorial type is to use the three variables $x_1,x_2,x_3$ from Section~\rif{sec:coords} for $T_1$,
...
coordinates provided by Section~\rif{sec:coords} to show that
...
In terms of the local coordinates of Section~\rif{sec:coords}, if $x_1=x_2=x_3=x_4=0$ defines the ice-ray dimer, then the explicit
small neighborhood we exclude is given by $|x_i|\le 0.01$, for $i=1,2,3,4$.  For example, in the pinwheel type on $T_1$, 
we have $(x_1,x_2,x_3)=(\alpha,\beta,x_\gamma)$ as given in Section~\rif{sec:pint}, and the fourth variable $x_4$ is determined
by the type of $T_0$.

\subsection{defining a curve}\label{sec:gamma}

In this stage, we define a curve $\Gamma$ in the configuration space of triple contact dimer pairs such that the ice-ray dimer
...
See Figure~\rif{fig:KKshift}.

\blankfig{KKshift}

In terms of the coordinates $(x_\alpha,\alpha)$ of Section~\rif{sec:coords} for two pentagons in contact, the relative position of $A$ and $C$
...
as described in Section~\rif{sec:auto-diff}.  This completes the reduction to points on the curve $\Gamma(t)$.


\subsection{global minimization along the curve}

The final stage of the proof of Theorem \rif{thm:dimer-area} is the minimization of the area of a $\Gamma$-dimer,
...
Thus, $b(T)=\area(T)$.  We have $\area(T) > \aK$ by Lemma~\rif{lemma:172}.  Since $T$ is not $b$-subcritical,
...
\begin{proof} 
 ...
 implies $m_2(T_2)=0$ by Lemma~\rif{lemma:n2m2}.
...
We claim that $m_1(T_2)=0$.  Otherwise, there exists $(T'_1,T_2)\in\PD$, and by Lemma~\rif{lemma:pd-b},
...
%Recall from above that we refine the clusters into clusters by
...
%cluster $\C$ is an equivalence class of Delaunay triangles under
...
By Lemma~\rif{lemma:t2b}, there does not exist a sequence $- \rab - \rab -$,
where the first triangle is obtuse.  Thus, we may assume that the first
triangle is nonobtuse.
By Lemma~\rif{lemma:no-ao}, there does not exist $T_1\rab T_2$,
...
\begin{proof} Assume for a contradiction that $T$ is nonobtuse.  By Lemma~\rif{lemma:no-ao}
...
Moreover, $b(T) = \area(T) -\epso n_2(T)$ by Lemma~\rif{lemma:obtuse-b}.
\end{proof}

Let $T\rab T_0$ be an arrow between two triangles in a cluster, where $T$ is obtuse.
...
In terms of the pentagons and angles in Figure~\rif{fig:bad}, we have
\[
...
An earlier lemma on pseudo-dimers (Lemma~\rif{lemma:pd-1}) gives that $b(T'_1)=\area(T'_1)\le \aK$.
...
with Lemma~\rif{lemma:pd-b}.  The claim ensues.
...
We are ready to give a proof of the pentagonal ice-ray conjecture. We repeat the statement
...
We combine the proof with a proof of Theorem~\rif{conj:main}.

\begin{proof}
By the main inequality in Lemma~\rif{lemma:main} applied to $a=\aK$, together with Remark~\rif{rem:equal}
it is enough to give a proof of Theorem~\rif{conj:main}.  Specifically, we show that
...
Figure~\rif{fig:theta}.  Let $x=x_\beta=\norm{\v_B}{\w_A}$ and $\beta =
...
Similarly, $\theta_{ABC}$ describes the inclination of a vertex of $A$ relative to the edge $(\c_A,\c_B)$.
It is a continuous function of $(x_\beta,\beta)$ if we fix its range at $[0,2\pi/5]$.  (Although the endpoints are
congruent modulo $2\pi/5$, continuity requires us to include both endpoints.)


In general, when two pentagons $A$ and $B$ come into contact, sometimes $A$ is the pointer and sometimes
$B$ is the pointer.  We describe an extended coordinate system $(x_\beta,\beta)$, with $\beta\in[0,4\pi/5]$ and $x_\beta\in[0,2\sigma]$
that unifies both directions of contact.  See Figure~\ref{fig:extended}.
When $\beta\le2\pi/5$, the coordinates are precisely as before.  Note that the configurations with $\beta=2\pi/5$
are ambiguous, with $B$ pointing into $A$ with coordinates $(x_\beta,\beta)$, or with $A$ pointing into $B$ with
coordinates $(x'_\beta,\beta')=(2\sigma - x_\beta,0)$.  When $\beta > 2\pi/5$, we let it represent the configuration
with $A$ pointing into $B$ and coordinates $(x'_\beta,\beta') = (2\sigma-x_\beta,\beta-2\pi/5)$.  The configuration
of two pentagons in contact depends continuously on the coordinates $(x_\beta,\beta)$.  The dependence is
analytic except along $\beta=2\pi/5$.

Using the continuous dependence of the configuration on the coordinates, we may uniquely
extend the functions $\theta_{BAC}$ and $\theta_{ABC}$ 
to the extended domain.  The functions still represent the inclination angle
of a vertex of $B$ (resp. $A$) with respect to the edge $(\c_A,\c_B)$.  However, when $\beta\ge 2\pi/5$, the
range of $\theta_{BAC}$ is $[0,2\pi/5]$ and the range of $\theta_{ABC}$ is $[\pi/5,3\pi/5]$.
An explicit formula for the extension is for $\beta\ge 2\pi/5$:
\begin{align*}
\dx{AB}(x_\beta,\beta) &=\dx{AB}(2\sigma-x_\beta,\beta-2\pi/5),\\
\theta_{BAC}(x_\beta,\beta) &= \theta_{ABC}(2\sigma-x_\beta,\beta-2\pi/5),\\
\theta_{ABC}(x_\beta,\beta) &= 2\pi/5+ \theta_{BAC}(2\sigma-x_\beta,\beta-2\pi/5).
\end{align*}

\blankfig{extended}

\subsection{two pentagons in contact, alternative coordinates}\label{sec:banana}

In this subsection, we describe how to describe a configuration of two pentagons $(A,B)$ in contact using the coordinates
$\dx{AB}$ and $\theta_{BAC}$ when pentagon $B$ is the pointer and $A$ is the receptor.  (In particular, we are not working
with extended coordinates in this subsection.)  Our computations depend on having
fast, numerically-stable algorithms for computing configurations in terms of these variables on intervals.  It is a matter of simple
trigonometry to express  $\theta_{ABC}$ in terms of $\dx{AB}$ and $\theta_{BAC}$.  However, it is somewhat more work to give
good interval bounds on $\theta_{ABC}$ in terms of interval bounds on $\dx{AB}$ and $\theta_{BAC}$.

\blankfig{banana2}

In general, there can be two configurations of pentagons for given $\dx{AB}$ and $\theta_{BAC}$. See Figure~\ref{fig:banana2}.
They can be distinguished in terms of coordinates $(x_\beta,\beta)$ by the sign of $h := x_\beta-\sigma$.
We will describe the configuration that is obtained when $h$ is non-negative.  By reversing orientation of the figure (that is, by
replacing  $C$ with $C'$ in the opposite half-plane), the variables transform according to the rule
\[
h \mapsto -h,\quad \theta_{BAC} \mapsto -\theta_{BAC},\quad \theta_{ABC}\mapsto -\theta_{ABC} (\mod 2\pi/5).
\]
Thus, by picking an appropriate orientation, the general case reduces to the case $h\ge0$.

We begin with a simple derivation of $\theta_{ABC}$ in terms of $\dx{AB}$ and $\theta_{BAC}$.
We write 
\[
\op{loc}(a,b,\psi) = c\ge 0,\quad \text{where } c^2 = a^2 + b^2 - 2ab \cos\psi
\]
...
\arc(a,b,c)
\]
for the angle of a triangle with sides $a$, $b$, and $c$ with $c$ the opposite edge.
Abbreviate $\ell = \dx{AB}$, $\theta=\theta_{ABC}$, $\theta'=\theta_{BAC}$.
Then referring to Figure~\ref{fig:theta-banana}, we compute $\theta=\theta_{ABC}$ as follows.
\begin{align*}
...
\theta &:= \alpha - \theta' (\mod 2\pi/5).
\end{align*}

\blankfig{theta-banana}

It can be checked that this computation correctly returns $\theta$ in the range $[0,2\pi/5]$
when we drop $(\mod 2\pi/5)$ from the last line.  Also, $\theta$ depends continuously
on $\ell$ and $\theta'$.  In the following discussion, the variables $\ell,\ell',r,h,x_\alpha,\alpha,\ldots$
retain their given meaning.

Not all pairs $(\ell,\theta')$ correspond to an configuration of touching pentagons.
...
Replacing $\theta$ and $\theta'$ by angles that are congruent modulo $2\pi/5$, we may assume that
...
We must run the pentagon existence test along the edges $AB$ and $BC$ to ensure that no pair
...
\subsubsection{pinwheel type with fixed shared edge}

For each triple contact type, we give further details about coordinate systems
adapted to a choice of a shared edge (in a dimer pair).  We call a $\Gamma$-triangle
to be a $P$-triangle that appears as one of the two triangles (with given shared edge) in the curve $\Gamma$
of Section~\rif{sec:gamma}.  When the the configuration space contains $\Gamma$-triangles,
we also describe a path from an arbitrary triangle in the configuration space to a $\Gamma$-triangle.

The configuration space of triple contact triangles is three dimensional.
The configuration space of  dimer in which both triangles have triple contact is four dimensional.
We parameterize this configuration space by three coordinates on each triple contact triangle, chosen
in a way that two of the coordinates are the same for both triangles: $4= 3+3-2$.
The two shared coordinates will have the form $(x_\alpha,\alpha)$ or $(x_\beta,\beta)$ or $(x_\gamma,\gamma)$.

A $\Gamma$-triangle is described by a single parameter $t$.  In each case, we describe $t$ as a function of the
coordinate system.
...
\subsection{triple contact at a pin-$T$ junction type}\libel{sec:pint} We describe a coordinate system on
...
\end{align*}
where the component-wise arithmetic operations on the right are computed by interval arithmetic.
Standard functions $F$ such as the square root and trigonometric functions are given an extension $F^D$ to automatic
differentiation.  For example,
\[
\op{sqrt}^D(f,f') = (\sqrt{f}, f'/(2\sqrt{f})),\quad \sin^D (f,f') = (\sin(f), \cos(f) f').
\]

Section~\rif{sec:dimer-pair} describes the proof of the local minimality of the ice-ray dimer.  We review that
...
in an explicit neighborhood of the ice-ray dimer.  In Section~\rif{sec:dimer-pair}, we give a curve $\Gamma$ 
...
Section~\rif{sec:calc} gives a sequence of inequalities that have been proved by MITM algorithms.
...
triangle $S$ in Lemma~\rif{calc:pseudo-area3}, the peripheral triangles are all subcritical.  We will prove
...
\begin{lemma}  We can assume without loss of generality that the triangle $S$ in Lemma~\rif{calc:pseudo-area3}
is $O2C$.
\end{lemma}
...
until it is a right triangle (because $O2C$ is assumed not to occur).  By Lemma~\rif{calc:pseudo2}, the shared edge
...
\subsubsection{Calculation~of Lemma~\ref{calc:pseudo2}} 
...
The central $T_0$ lies in a five-dimensional configuration space. ($P$-triangles form a six-dimensional space,
...
\subsubsection{Calculation~of Lemma~\ref{calc:good}} %FHB.
...
\subsubsection{Calculation~of Lemma~\ref{calc:pent3}} %HUQ.
...
\subsubsection{Calculation~of Lemma~\ref{calc:pent4}} 
