...
Lemma~\rif{lemma:main}, it implies the main result, Theorem
\rif{thm:main}.  The proof of this result appears at the end of
...
The second figure (which is degenerate of type $L$) does not have
type $\Delta$.}{
[scale=0.5]
\pen{0}{0}{90};
\draw (0,0) node {$B$};
...
\begin{definition}[$T$ and $LJ$-junction]
...
%\area(T) \ge \area(2 \kappa,2 \kappa,\kappa\sqrt{8}) = 2 \kappa ^2 \approx 1.30902 > \acrit.
...
\begin{proof}
  ...
  Lemma~\rif{calc:pseudo1}.  It has length at least $1.72$ by
  Lemma~\rif{lemma:172}.  If the third edge has length at least $1.8$,
...
This is easy to carry out.  By Lemmas~\rif{lemma:primary} and
...
with triple contact in Section~\rif{sec:contact}.  In this stage, we
...
Section~\rif{sec:coords} to show that these conditions force $T_0$ to
...
\begin{proof}  
  ...
  \acrit$ by Lemma~\rif{lemma:172}.  Since $T$ is not $b$-subcritical,
...
\begin{lemma}\libel{lemma:m2-dimer}  
  Let $(T_1,T_0)\in DP$ and let $(T'_1,T'_0)\in \PD$.  Then
  $m_-(T_0)=m_-(T_1)=0$.
%$T'_0\nRa T_0$ and
%$T'_0 \nRa T_1$.  In particular, 
\end{lemma}
...
\begin{proof} 
  ...
  $\area(T_0) > \acrit$ by Lemma~\rif{lemma:right}.  Thus, we must
...
\begin{lemma} \libel{lemma:rab-sequence} There does not exist a
...
\begin{proof}[Proof of dimer theorem \rif{thm:dimer}.]
  ...
  $m_+(T)=m_-(T)=0$ in Lemmas~\rif{lemma:m1-dimer} and
  \rif{lemma:m2-dimer}.
...
\begin{proof}  By Lemma~\rif{lemma:n2m2}, $n_-(T)=0$.
...
\begin{lemma}\libel{lemma:Nb}  If $(T_+,T_-)\in \N$, then $b(T_+)>\acrit$.
\end{lemma}
...
By Lemma~\rif{lemma:no-ao}, there does not exist $T_1\rab T_0$,
...
\begin{theorem}\libel{lemma:obtuse}  
  ...
  is finite, and the strict main inequality (\rif{eqn:strict-main})
  holds for $\C$.
\end{theorem}
...
% Let $T_1\rab T_0$ be an arrow between two triangles in a cluster
% that contains an obtuse triangle.  By the structure of clusters
% (Lemma \ref{lemma:c-weak}), we may assume that $T=T_1$ or $T=T_0$.
% Next, we consider the case $T=T_0$.  If $T_0$ is obtuse, then by
% similar reasoning, it is enough to show that $\tn(T_0)\ge
% n_{ext,-}(T_0)$.
...
(\rif{eqn:n}) is satisfied (for each $\tn$).
...
\begin{proof}  
  ...
  $n_+(T)=n_-(T)=0$.  By Lemma~\rif{lemma:m1m2}, we have $m_+(T)=0$.
  By Lemma~\rif{lemma:3m2}, we have $m_-(T)\le 2$.
...
\rif{def:large} and Lemma~\rif{lemma:large}.)  Lemma
...
\begin{proof}  
  ...
  lemma, we have $n_-(T_0)=0$.  By Lemma~\rif{lemma:m1m2}, we have
  $m_-(T_0)=0$.  By Lemma~\rif{lemma:n1m1}, we have $n_+(T_0)=0$.
...
\begin{proof}  
  ...
  \area(T_1^i)\le \acrit$ by Lemma~\rif{lemma:T1-all}.  By Lemmas
  \rif{lemma:n2-0} and \rif{lemma:m1-0}, we have
  $m_+(T_0)=n_-(T_0)=0$.
...
\begin{proof}  
  ...
  Lemma~\rif{lemma:pd-1}, $b(T_1)=\area(T_1)\le \acrit$.

  We assume that $\C$ is not a dimer pair $(T_1,T_0)$.

  We consider the case of a pseudo-dimer.  If $(T_1,T_0)\in\PD$, then
  $m_-(T_0)=n_-(T_0)=0$ (by Lemmas \rif{lemma:pd-m2} and
  \rif{lemma:pd-n2}).  Thus, by Lemma~\rif{calc:pseudo-area}
\[
b(T_1)+b(T_0) 
...
We consider the case $m_-(T_0)= 3$.  In fact, Lemma~\rif{lemma:3m2},
...
covered by Lemma~\rif{calc:large}.  That lemma implies
\[
b(T_1) + b(T_0) 
...
\begin{proof}
  ...
  give a proof of Theorem~\rif{conj:main}.  Specifically, we show that
  ...
  Theorem~\rif{thm:dimer}.  If $\C$ is not a dimer pair, then we show
  ...
  $4$, $3$, $2$, or $1$, this is found in Lemmas \rif{lemma:4},
  \rif{lemma:3}, \rif{lemma:2}, \rif{lemma:singleton}.
...
Similarly, $\theta_{ABC}$ describes the inclination of a vertex of $A$
relative to the edge $(\c_A,\c_B)$.  It is a continuous function of
$(x_\beta,\beta)$ if we fix its range at $[0,2\pi/5]$.  (Although the
endpoints are congruent modulo $2\pi/5$, continuity requires us to
include both endpoints.)

In general, when two pentagons $A$ and $B$ come into contact,
sometimes $A$ is the pointer and sometimes $B$ is the pointer.  We
describe an extended coordinate system $(x_\beta,\beta)$, with
$\beta\in[0,4\pi/5]$ and $x_\beta\in[0,2\sigma]$ that unifies both
directions of contact.  See Figure~\ref{fig:extended}.  When
$\beta\le2\pi/5$, the coordinates are precisely as before.  Note that
the configurations with $\beta=2\pi/5$ are ambiguous, with $B$
pointing into $A$ with coordinates $(x_\beta,\beta)$, or with $A$
pointing into $B$ with coordinates $(x'_\beta,\beta')=(2\sigma -
x_\beta,0)$.  When $\beta > 2\pi/5$, we let it represent the
configuration with $A$ pointing into $B$ and coordinates
$(x'_\beta,\beta') = (2\sigma-x_\beta,\beta-2\pi/5)$.  The
configuration of two pentagons in contact depends continuously on the
coordinates $(x_\beta,\beta)$.  The dependence is analytic except
along $\beta=2\pi/5$.

Using the continuous dependence of the configuration on the
coordinates, we may uniquely extend the functions $\theta_{BAC}$ and
$\theta_{ABC}$ to the extended domain.  The functions still represent
the inclination angle of a vertex of $B$ (resp. $A$) with respect to
the edge $(\c_A,\c_B)$.  However, when $\beta\ge 2\pi/5$, the range of
$\theta_{BAC}$ is $[0,2\pi/5]$ and the range of $\theta_{ABC}$ is
$[\pi/5,3\pi/5]$.  An explicit formula for the extension is for
$\beta\ge 2\pi/5$:
\begin{align*}
\dx{AB}(x_\beta,\beta) &=\dx{AB}(2\sigma-x_\beta,\beta-2\pi/5),\\
\theta_{BAC}(x_\beta,\beta) 
&= \theta_{ABC}(2\sigma-x_\beta,\beta-2\pi/5),\\
\theta_{ABC}(x_\beta,\beta) 
&= 2\pi/5+ \theta_{BAC}(2\sigma-x_\beta,\beta-2\pi/5).
\end{align*}
...
$\theta_{BAC}$ when pentagon $B$ is the pointer and $A$ is the
...
trigonometry to express $\theta_{ABC}$ in terms of $\dx{AB}$ and
$\theta_{BAC}$.  However, it is somewhat more work to give good
interval bounds on $\theta_{ABC}$ in terms of interval bounds on
$\dx{AB}$ and $\theta_{BAC}$.

\blankfig{banana2}

In general, there can be two configurations of pentagons for given
$\dx{AB}$ and $\theta_{BAC}$. See Figure~\ref{fig:banana2}.  They can
be distinguished in terms of coordinates $(x_\beta,\beta)$ by the sign
of $h := x_\beta-\sigma$.  We will describe the configuration that is
obtained when $h$ is non-negative.  By reversing orientation of the
figure (that is, by replacing $C$ with $C'$ in the opposite
half-plane), the variables transform according to the rule
\[
...
We begin with a simple derivation of $\theta_{ABC}$ in terms of
$\dx{AB}$ and $\theta_{BAC}$.  We write
\[
...
opposite edge.  Abbreviate $\ell = \dx{AB}$, $\theta=\theta_{ABC}$,
$\theta'=\theta_{BAC}$.  Then referring to
Figure~\ref{fig:theta-banana}, we compute $\theta=\theta_{ABC}$ as
follows.
\begin{align*}
...
Replacing $\theta$ and $\theta'$ by angles that are congruent modulo
...
For each triple contact type, we give further details about coordinate
systems adapted to a choice of a shared edge (in a dimer pair).  We
call a $\Gamma$-triangle to be a $P$-triangle that appears as one of
the two triangles (with given shared edge) in the curve $\Gamma$ of
Section~\rif{sec:gamma}.  When the the configuration space contains
$\Gamma$-triangles, we also describe a path from an arbitrary triangle
in the configuration space to a $\Gamma$-triangle.

The configuration space of triple contact triangles is three
dimensional.  The configuration space of dimer in which both triangles
have triple contact is four dimensional.  We parameterize this
configuration space by three coordinates on each triple contact
triangle, chosen in a way that two of the coordinates are the same for
both triangles: $4= 3+3-2$.  The two shared coordinates will have the
form $(x_\alpha,\alpha)$ or $(x_\beta,\beta)$ or $(x_\gamma,\gamma)$.

A $\Gamma$-triangle is described by a single parameter $t$.  In each
...
will be assume to consist of one triangle (called the central
...
The aim of the algorithm is to give lower bounds on the sum of the
...
triangle $T_0$ shared an edge and two pentagons $A$, $B$ with a
...
Each $P$-triangle is a point in a six dimensional space (described by
three triangle edge lengths and three inclination angles of the
pentagons at its vertices).  A cluster consisting of one central
$P$-triangle and $k$ flanking triangles is a point in a space of
dimension $6 + 3k$.  If we cover the space with cubes of edge-length
$\epsilon$, then there are order $(1/\epsilon)^{6+3k}$ cubes.  This is
beyond our computational reach when $k>0$.

We can use MITM techniques to reduce to order $(1/\epsilon)^6$ cubes,
and this puts the computations (barely) within the reach of a laptop
computer.  Specifically, we fix a edge size $\epsilon$ and cover the
space of peripheral triangles by cubes of size $\epsilon$, calculating
area, edge lengths, inclination angles, and other relevant quantities
...
endpoints, which are indexed by integers.  These integer indices
become the keys to the hash table.  For each key, the hash table
records the smallest area of a peripheral triangle with that key.

Once the hash is created, we divide the space of central triangles
into cubes and computes the relevant quantities (edge lengths,
triangle area, and inclination angles) over each cube using interval
arithmetic.  Bounds on the peripheral triangles are recovered from the
hash table to get a lower bound on the sum of the areas of the
...
the smallest $\epsilon$ that was required was approximately $0.00024$.
...
such as numerical approximation by a difference quotient $f(x + \Delta
x)/\Delta x$, symbolic differentiation (as in computer algebra
...
represented as a pair $(f,f')$ of intervals, where $f$ is an interval
bound on the function, and $f'$ is an interval bound on the derivative
of the function.  More complex expressions can be built from simpler
expressions by extending arithmetic operations $(+)$, $(-)$, $(*)$,
$(/)$ to pairs.  For example,
\begin{align*}
(f,f') + (g,g') &= (f+g,f'+g'),\\
(f,f') * (g,g') &= (f g,f g' + f' g),\\
...
computed by interval arithmetic.  Standard functions $F$ such as the
square root and trigonometric functions are given an extension $F^D$
to automatic differentiation.  For example,
\[
\op{sqrt}^D(f,f') = (\sqrt{f}, f'/(2\sqrt{f})),\quad \sin^D (f,f') = (\sin(f), \cos(f) f').
\]

Section~\rif{sec:dimer-pair} describes the proof of the local
...
Section~\rif{sec:dimer-pair}, we give a curve $\Gamma$ (in the space
...
\begin{proof} 
  ...
  occur).  By Lemma~\rif{calc:pseudo2}, the shared edge with $T_0$ has
...
\subsubsection{Calculation~of Lemma~\ref{calc:large}} %FHB.
