...
%  $\acrit$.  The longest edge of an acute subcritical Delaunay triangle
%  cannot be less than $1.72$.   (See Lemma~\rif{lemma:172}.)  See
...
It is equalateral with edge length about $1.72256$.}
{
...
\tikzfig{pseudo-dimer}{Pseudo-dimer.  There exist pairs of acute
  ...
  pseudo-dimer has area about $2\acrit - 10^{-5}$, and the triangle $T_1$
...
%\draw (6,0.8) node {$\area(T_1)< \acrit$};
%\area(T_0)+\area(T_1)< 2\acrit\end{align*}};
\end{scope}
}
...
%\coordinate (Aa) at (50.58:1);
%\coordinate (Ab) at (50.58+72:1);
%\coordinate (B) at (1.1,1.49);
%\coordinate (Ba) at ($(B)+ (181.07:1)$);
...
\item We deform so that each of $T_0$ and $T_1$ is $O2C$.
  % \item We deform so that both triangles are triple contact, or so
  %   that both are $O2C$ and $T_1$ is long isosceles.
...
\begin{proof} 
  ...
  right half-plane, and with $\c_B$ in the positive half-plane (Figure~\rif{fig:slider}).  Our
  ...
  %distance $\dx{CB} \ge\kappa\sqrt8$ and again this implies that
  %the triangle is not subcritical.
\end{proof}

\tikzfig{slider}{The center $\c_B$ of the nonobtuse triangle lands
in the first quadrant and gives a triangle $(\c_A,\c_B,\c_C)$ of height and base
both at least $2\kappa$.}{
[scale=1.0]
\draw (0,0) -- (2,0);
\draw (0,0) -- (0,2);
...
% Assume that $\c_A$ and $\c_C$ lie along the $x$ axis, with $\c_A$ to
% the left of $\c_C$.  Let $B$ and $D$ be the two outer pentagons.  We
% deform this dimer by fixing $A$, and translating $C$ vertically
% (maintaining the pointer contact of $A$ to $C$).  We translate $B$
% and $D$ to maintain all edge to edge contacts: $(A,B)$, $(A,D)$,
% $(C,B)$, $(C,D)$.
...
% vertical translation distance of the pentagon $C$.
...
by Lemma~\rif{lemma:172}.  According to the definition of $\M$, we
