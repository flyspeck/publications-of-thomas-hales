...
%such that its boundary is given by a path $g:[0,t_f]\to\SL$ as above)
...
\item The image of $z$ lies in $\hstar\subset \h$ (see Section
  ...
  \item The endpoints of the trajectory are $(g(0),z(0))=(I,z_0)$ and
    ...
    \exp(J\pi/3)$.  
   %The trajectory $z$ extends to a rotationally
    %symmetric trajectory (\ref{eqn:rotsym}).
\item the path
...
%  \frac{3}{2} \int_0^{t_f} c_{11}(g) c_{21}(g)' - c_{21}(g) c_{11}(g)'
%  + c_{12}(g) c_{22}(g)' - c_{22}(g) c_{12}(g)' dt \to \min,
\end{equation}
%where
%\[
%g =  \begin{pmatrix} c_{11}(g) & c_{12}(g) \\ 
%c_{21}(g) & c_{22}(g) \end{pmatrix} : [0,t_f]\to \SL.
%\]
...
in the velocity set.
%$f(x,It is enough to
%check that the segment in $\ring{R}^2$ from $f(x,y;u_1)$ to
%$f(x,y;u_2)$ lies in the velocity set. We may assume that
%$f(x,y;u_1)\ne f(x,y;u_2)$.  
...
%Then $L(f(x,y;u))$ is zero along
%the segment $[u_1,u_2]$.  This proves the convexity of the velocity
%set.  (Closer analysis should show that for each $x+iy\in \h$
%satisfying the star inequalities, the convex velocity set is a
%triangle in $\ring{R}^2$.)
...
% (\ref{eqn:adjoint}) for the maximized
%  Hamiltonian (\ref{eqn:pmp-max}).
...
%but it appears in the accompanying computer
%algebra code.  In brief, the isomorphism $\rho:\h\to \h$ induces an
%isomorphism of the cotangent space $\rho^*:T^*\h\to T^*\h$ in the
%usual way, and this determines how $\nu$ must
%transform:
%\[
%\langle (\bar \nu), D\rho_{* z_0} v\rangle_{\rho.z_0}
%= \langle\nu,v\rangle_{z_0},
%\quad \forall v\in T_{z_0} \h.
%\]
...
z(t+t_f) = R^{-1}.z(t).
\end{equation}
%This states that the trajectories in the upper half-plane have
%rotational symmetry by $R^{-1}$.  
...
%We have the following points of comparison between the smoothed
%$6k-2$-gon $D_{6k-2}$ and the smoothed $6k+2$-gon $D_{6k+2}$.  
...
\ref{fig:cost}). 
%The values at $n=4+,8,10,14,16,20,22$ are exact, and
%a Bezier curve interpolates between those points.
...
%
%By the monotonicity of the Hamiltonian (see Section
%  \ref{sec:bang}), the maximum of the Hamiltonian occurs at a subset
%  of the vertices of the control simplex $U$.  If there is a singular
%  arc, then the maximum is achieved at two or three of the vertices of
%  the control simplex.  
...
%We show  that $U_{\lambda(t)}$ is not an edge on any interval.

%We return to the case where $U_{\lambda}$ is an edge of $U$ along some
%time interval.  
...
%
%  many zeros of some switching function on
%  every sufficiently small neighborhood of $t=0$.  Then there must
%  also be infinitely many switches to the third vertex on every
%  sufficiently small neighborhood of $t=0$.
\end{theorem}

\begin{proof} 
%By symmetry, we may assume that the switching function
%  is $\chi_{23}$.  To prove the contrapositive, we assume that there are
%  only finitely many zeros of the switching functions $\chi_{12}$ and
%  $\chi_{13}$ and show that there are only finitely many zeros of
%  $\chi_{23}$.  
%
...
%We remark that in a neighborhood of the point in question $z=i\in \h$,
%many aspects of the problem become quite simple.  Write $x = \epsilon
%x_1$ and $y= 1+\epsilon y_1$ with $\epsilon$ small.  The cost function
%over interval $[0,t_f]$ is approximately the problem of minimum time:
%\[
%\op{cost}(z) = 3 t_f + O(\epsilon^2).
%\]
%The ODE (\ref{eqn:ode-state-upper}) becomes (to first order in
%$\epsilon$):
%\[
% (\epsilon x_1',\epsilon y_1') 
%\in \op{convex-hull} \{(-2,0),(1,-\sqrt{3}),(1,\sqrt{3})\}.
%\]
%That is, the control problem constrains the derivatives to lie in an
...
\]
%where $\ring{P}^+$ identifies nonzero tuples that differ by a positive
%multiplicative factor along fibers of the cotangent bundle.  The extra
%factor of $\ring{R}_{cost}:=\ring{R}$ is there to include $\lambda_{cost}\le 0$.  A careful
%treatment of the working hypothesis (\ref{hyp:star}) would allow us to
%%replace $\hstar$ with an explicit compact subset $K\subset \hstar$.  A
%careful treatment of the second working hypothesis would allow us to
%remove an explicit open neighborhood of the initial conditions of the
%circle.
...
%The problem has been reduced to an optimization problem over a a
%five-dimensional manifold.  There is a standard cover of projective
%space $\ring{P}(\ring{R}^k)$ by compact sets that is useful for
%computations: we use coordinates $(y_1,\ldots,\hat
%y_i,\ldots,y_n)=(x_1/x_i,\ldots x_n/x_i)$ with $|y_i|\le 1$ on the
%subset of projective space for which $|x_i|$ has the largest absolute
%value.
