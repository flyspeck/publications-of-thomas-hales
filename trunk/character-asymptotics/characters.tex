...
Suppose that \(\mathcal R\) is a collection of positive real
numbers.
Suppose that \(G\) is \(\unfield\)-split.
	\Lxxx{Is this enough to get a hyperspecial point?
Maybe we need even \(\field\)-quasi-splitness for that.
Anyway, even if we don't worry about hyperspecial points,
then we still have some unpleasantness from replacing
each \(\R/\Z\) by \(\R/e^{-1}\Z\), where \(e\) is the
ramification degree of (the splitting field of) the
corresponding root.}
We put
\[
\motB(G) = (\R/\Z)^\Simple \times 2^W.
\]
For any alcove \(C \subseteq \BB(G)\),
we define a map \(\BB(G) \to \motB(G)\) as follows.
For \(x \in \BB(G)\), there is a unique point \(x'\) in the
intersection with \(G\dota x\) of the closure of \(C\).
Then we associate to \(x\) the pair \((r, S)\), where, for
each \(\alpha \in \Simple\), \(r_\alpha\) is the value at
\(x\) of any affine root with gradient \(\alpha\),
and \(S\) is the set of all those elements of \(W(G, S)\) that
have a representative in \(N_G(S)\) fixing \(x'\).
