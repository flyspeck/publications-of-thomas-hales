...
Suppose that \(G\) is \(\unfield\)-split.
	\Lxxx{Without unramifiedness, we have some
unpleasantness from replacing each \(\R/\Z\) by
\(\R/e^{-1}\Z\), where \(e\) is the ramification degree of
(the splitting field of) the corresponding root.}
We put
\[
\motB(G) = (\R/\Z)^\Simple \times 2^\affW.
\]
If \(\bS\) is a maximal split torus in \(\bG\)
and \(C\) is an alcove in \(\AA(S)\),
then we define a map \(\BB(G) \to \motB(G)\) as follows.
For \(x \in \BB(G)\), there is a unique point \(x'\) in the
intersection with \(G\dota x\) of the closure of \(C\).
Then we associate to \(x\) the pair \((r, W')\), where, for
each \(\alpha \in \Simple\), \(r_\alpha\) is the value at
\(x\) of any affine root with gradient \(\alpha\),
and \(W'\) is the set of all those elements of \(\affW(G, S)\) that
fix \(x'\).
...
element).  In \cite{spice:coefficients}, the `peeling'
process from \(\pi\) to \(\pi'\) keeps the multiplicative
character \(\phi'\) on \(\pi'\), so that we would expect our
\(\pi_0\) to be twisted by \(\prod_{i = 0}^{d - 1} \phi_i\)
and our \(\cO^{G^0}(0)\) to be replaced by
\(\cO^{G^0}(\Gamma)\);
