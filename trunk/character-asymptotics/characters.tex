...
Let \(\mathcal F(C)\) be the set of (open) facets of
(the closure of) \(C\).
Write \(\affW_C(G, S)\) for the (finite) set of elements
...
\Bigr) \times 
	...
	\mathcal F(C),
\]
and define a map \(\BB(G) \to \motBwith{S, C}(G)\) as follows.
For any \(x \in \BB(G)\), there is a unique point \(x'\) in the
intersection with \(G\dota x\) of the closure of \(C\).
	\citeme
Then we associate to \(x\) the triple \((r, W', F)\),
where
	\begin{itemize}
	\item for each \(\alpha \in \Phi(\bG, \bS)\),
\(r_\alpha\) is the value at \(x\) of any affine root with
gradient \(\alpha\),
		\citeme
	\item \(W'\) is the set of all those elements of
\(\affW(G, S)\) that fix \(x'\),
and	\item \(F\) is the facet of \(C\) containing \(x'\).
	\end{itemize}
