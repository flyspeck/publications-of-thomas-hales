...
% 
% To appear in the proc. of the Rutgers conference in memory
...
\topmatter
\title Hyperelliptic Curves and Harmonic Analysis\endgraf
      {\it (Why harmonic analysis on reductive 
                       p-adic groups is not elementary) } \endtitle
...
\subjclass 22E50; Secondary 22E35, 14H52, 11G20\endsubjclass
\abstract  
...
\thanks Research supported by an NSF postdoctoral fellowship \endthanks
\endtopmatter
...
\def\C{{\Bbb C}}
\def\O{{\Cal O}}
\def\Ima{{\Cal I\hskip-.2em m}}
\def\bGamma{\bar\Gamma}
\def\diag{\hbox{diag}}
\def\bk{\bar k}
\def\Gal{\operatorname{Gal}}
\def\bF{{\bar F}}
\def\vvol{\operatorname{vol}}
...
\head 1. Characters and Curves\endhead
...
\footnote""{This paper is in final form, and no version of it
...
\head  2.  The Construction of Hyperelliptic Curves\endhead
...
\definition{Notation}  We define $\sigma^*_w (B_1,B_2) = (\sigma_w B_1',\sigma_w B_2')$,
...
\demo{Proof (Lemmas 2.1 and 2.2)}  \!The definitions show directly that
...
\demo{Proof (Sketch)}  The surface $S$ is
...
\subhead Example 2.6 \endsubhead The second example we consider is $G=SO(8)$.  We write
...
\demo{ Proof}  The same argument as in the proof of the previous lemma 
...
\subhead Example 2.8\endsubhead  Consider the group $G=SO(2n+1)$.  We have the simple
...
\head 3. Relation to Shalika Germs\endhead
...
\demo{ Proof}  If $t$ is half-integral
...
Then 
$$\int_{\P^1(F)}\eta(P(x))\left| {dx\over x^2}\right| =
   {-C(P)\over q},$$
where 
...
\demo{Proof}  As in \cite{H5,2.2}, we may replace the integrand $\eta(P(x))$ by 
...
\demo{Proof}  By \cite{H5,1.2}, the stable subregular Shalika germs are given by
...
\demo{Proof}   For the stable germs, there is no harm in working on the
...
by Lemma 4.7 to obtain the theorem.\hfill\hbox{}\hfill\qed\enddemo
...
\demo{Proof}  The argument is similar to the proof of the
...
\head 5. The Unit Element of the Hecke Algebra\endhead
...
\demo{Proof}  If $\theta:F^\times\to\C^\times$ is a quadratic
...
\demo{Proof (Theorem 5.1)}  The Shalika germ expansion of the stable
...
\head 6. Fourier Transforms and Characters\endhead
...
$$\hat\mu_\O(X) = \sum_\pi \hat c_\O(\pi)\chi_\pi(\exp(X)),$$
for $X$ sufficiently small.
\endproclaim

\demo{Proof}  Kazhdan \cite{K,\S2} proves that there is a cuspidal function
...
two results combine to give the lemma.\qed\enddemo
...
\demo{Proof}  The local character expansion on the half-integral set
will in general contain the term $C_3(\tau_E(t))$ of Theorem 6.4.\qed\enddemo
...
\demo{Proof (Theorem 6.4)}  We write the four Shalika germs of subregular
...
$$H=\left\{\pmatrix *&&*\\&*&\\*&&*\endpmatrix\right\}.$$
...
\demo{Proof}  This follows easily from the results of \cite{H2}.  We sketch
the argument.  There
...
\head  7.  Proofs and Conclusion\endhead
