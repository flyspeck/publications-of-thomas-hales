...
that the number of stable distributions on the product of
$\fh_{z(\lambda_i)}(F_1)$ does not depend on $\lambda\in
\Lambda^r(F_1)$, since different choices of $\lambda$ merely permute
the factors $\fh_{z(\lambda_i)}$.  The cardinality of $L^+$ is
$k'_1\cdots k'_r$, the number of stable distributions support on the
nilpotent cone on the product of the $\fh_{z_i}$.  We invoke
Lemma~\ref{thm:tensor}(1) to see that for all $i$, the functions
...
%% Do we need compactness here? XX.
...
have checked that we have an $a_0$-asymptotic relation relating with
function with index $\bar\ell_i$ to the others for each
$i=1,\ldots,r$, and by taking tensor products of the relations we
obtain a linear relation.  This gives the claim.
...
\begin{lem} Let $G$ and $G'$ are forms of a simple, simply connected
  ...
  fixed choices can be chosen to be equal.  Then $G$ and $G'$ fall into one of the
  three cases just enumerated.
\end{lem}
...
$\phi(\op{qFr})=1$ (see \cite[\S7]{Gross}).
