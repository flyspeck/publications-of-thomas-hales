...
\blue{\subsection{Acknowledgement} J.G. is grateful to  
Radhika Ganapathy for suggesting this project and helpful conversations at the beginning;
we also thank MSRI where this collaboration was started. }
...
We use the concepts of {\it definable sets} and {\it
  constructible motivic functions} (or constructible functions for
short) from \cite{CL}.  All constructible functions in this paper are
understood in this sense. \blue{In fact, in \cite{CL} a closely related notion of \emph{definable subassignment} is used in the place of our notion of a \emph{definable set}, which does not change the meaning. }
...
nonarchimedean fields whose residue field characteristic is at least
...
\begin{proof} \blue{First, we show that (1) and (2) are first order statements in the 
  Denef-Pas language, which would imply, by a transfer principle, that it is enough to prove
  the result when $F$ has characteristic zero.  
  The statement (2) is first-order since the set $G_{z,\s,0+}$ is definable, by 
  \cite{CGH2}*{Lemma 3.4}. The set  $G_{z,\s,0}$ appearing in (1) is not yet known to be definable in general, but it is contained in a slightly larger definable set of Galois-fixed points of 
  $G(K)_{\s, 0}$, which we will denote by $G_\s^0$ (where $K$ is an extension that splits $G_z$), by the same lemma. 
  However, if $u$ is a unipotent element contained in $G_\s^0$, then in fact $u$ is contained in $G_{z, \s, 0}$ by \cite{debacker:nilp}*{Lemma 4.5.1}. 
  Thus (2) is a first-order statement as well. }
   
  
  \blue{Now we} work over $F$, with
  $z\in Z(F)$ fixed, and write $\fg_{\s,r}$, $\fg_{\s,r+}$, $G_{\s,r}$,
  $G_{\s,r+}$ for the usual Moy-Prasad filtrations with
  $r\in\ring{R}$ \blue{for the Lie algebra, and $r\in \ring{R}_{\ge 0}$ for the group}.  In particular, $\fg_{\s,0}=\fg_{z,\s,0}(F)$, and so
  forth.
 ...
 \end{enumerate}
  \blue{This follows from \cite{kim:hecke}*{Prop. 3.1.1}, which shows that the 
  exponential map defined by the usual series converges, and from the properties of 
  mock exponential maps constructed by J. Adler \cite{adler:anisotropic}*{\S 1.5-1.6} combined  
  with the fact that the exponential map when it is defined is a mock exponential map.}
...
\[
\blue{\locus{g}_F(t) = \sum_{i=1}^n c_i t^{k_i} q_F^{\ell_i t},}
\]
where \blue{$q_F$ is the cardinality of the residue field of $F$}, and each $c_i$ depends only on residue field variables, the integers
\blue{$k_i$ and $\ell_i$} do not depend on any variables in the Denef-Pas
language, and we can assume that $(k_i,\ell_i)$ are mutually different
for different $i$.
...
and follows from \blue{$o$-minimality of the structure of the real field enriched with} the exponential.
