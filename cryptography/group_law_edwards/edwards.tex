...
$\phi$.  
%Geometrically, $\bar e_i=0$ asserts that $(a_i,b_i)$ is a
%point on the Edwards curve.  More generally, 
...
%\[
%(g_1,g_2)\equiv (0,0) \mod \{e_1,e_2,e_3\}.
%\]
\end{proof}
...
%acts as automorphisms of $\Eoo$.  We obtain a binary operation on the
...
%Let $\Eaff$ be the set of solutions of 
Equation (\ref{eqn:t}) in $k^2$.
% with
%$(x,y)\in k^2$.
Let $\Eoo\subset \Eaff$ be the subset of $\Eaff$
% solutions of Equation (\ref{eqn:t}) with
nonzero coordinates $x,y\ne0$.
%and $\Eaff,\Eoo$ be the set of $k$-points of $\Eaff,\Eoo$, respectively.
%We write $P\in \Eaff$ as shorthand for $P\in \Eaff(k)$.
...
%  $G$ acts without fixed point on $\Eoo$.  That is, $g z = z$ implies
%  that $g=1_G\in G$.
%\end{lemma}

%\begin{proof}  Write $z=(x,y)$.
...
%modulo $2$, writing $\oplus_i$ with $i\in\ring{F}_2$.
...
%whenever we have more than one composition law to choose from.
...
% when , but
%the formula (\ref{eqn:sum}) still makes sense and puts a group structure on
%the hyperbola, which in the projective case is a projective line.
