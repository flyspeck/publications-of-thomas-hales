...
construction, when applied to a curve other than the circle, can also lead to a group law on the curve. In particular, we obtain a group law for a large class of Edwards
...
We write $\iota(z) = (x,-y)$, where $z=(x,y)$, for the operation on ordered pairs corresponding to complex conjugation.
...
is $\iota(z_3)$, and this gives a geometrical construction
of the group law. 
...
in terms of $z_1$ and $z_2$.  This calculation rediscovers the recently forgotten standard formula (\ref{eqn:cx}).
...
\tikzfig{fig1}{A unit circle with center $(0,0)$
...
We replace the unit circle with a more general algebraic curve $C$.  
...
Most writers prefer to restrict to curves of genus $1$
...
in all cases.  We prove below a completeness result (Lemma~\ref{lemma:complete}) showing that this formula works
...
reduces to the formula (\ref{eqn:cx}) for the complex multiplication (as it should).
...
Borrowing the definition from the previous section of hyperbolic addition,
...
word \emph{complete} in this article  is that of the lemma: the affine plane contains all $k$-points, 
...
This condition could be relaxed to include degenerate cases, but we do not do so here.
...
has degree four and is separable.  If the quartic has a root in the field $k$,
we may apply a Moebius transform in $y$ 
that maps the root
to infinity, to transform the curve into Weierstrass form: $w^2$ equals a separable cubic
polynomial in $y$.
...
portion of all elliptic curves.  Other writers sweat much more than we have to prove the group
...
\section{Formalization in HOL Light}\label{sec:hol}
...
elliptic-curve cryptography at the 31st Chaos Communication Congress on December 27, 2014.
