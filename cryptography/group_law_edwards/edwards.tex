...
such that $\bar d$, the image of $d$, is not a nonzero square and such that 
...
Let $g = (1 - c d x_1^2 x_2 ^2) (1 - d x_1^2 y_2^2)$.   
We have
...
where
\begin{equation}
r_0 = (1-d x_1^2),\quad r_1 = d^2 x_1^2 x_2^2 y_2^2,\quad r_2 = d x_1^2.
\end{equation}
The kernel property gives $\phi(g) = (1 - \tau^2 \bar d a_1^2 a_2^2)(1 - \bar d a_1^2 b_2^2) =0$.
We solve this equation for $\bar d$, which expresses it as the square of $1/(\tau a_1 a_2)$ or $1/(a_1 b_2)$.
\end{proof}

We are ready to state and prove the main result of the paper.  The group law is expressed generally enough
to include the circle (and ellipse) as a special case $\bar d = 0$.  Cryptography textbooks are emphatic
that elliptic curves are not ellipses, but here we give a unified treatment of both group laws!  
Bernstein and Lange
gave a beautiful elliptic-curve tutorial from this unified point of view at the 31st Chaos Communication Congress.



\begin{theorem}[group law]\label{thm:group} 
Let $k$ be a field (of char$\ne 2$), let $\bar c \in k^\times$ be a square, and let $\bar d\in k\setminus k^{\times 2}$.
